...
export function scoreProduct(product: {
  ...
  const marginScore = priceNum >= 50 ? 25 : priceNum >= 30 ? 20 : priceNum >= 15 ? 15 : priceNum >= 5 ? 10 : 5;

  // Trend score (0-20): based on category hotness
  const hotCategories = ["yoga", "fitness", "wellness", "skincare", "pet", "baby", "home", "tech", "gadget", "beauty"];
  ...
  const detectedCategory = Object.keys(trendingCategories).find(cat => nameAndNiche.includes(cat));
  ...
  const total = marginScore + trendScore + competitionScore + categoryScore + descScore + reviewScore;
