            ...
            {REGION_LIST.map(r => (
              <button
                ...
                style={{
                  ...
                  background: regionCode === r.code ? 'rgba(59,130,246,0.15)' : 'transparent',
                  border: 'none', cursor: 'pointer', textAlign: 'left' as const,
                  transition: 'background 100ms',
                  borderLeft: regionCode === r.code ? '3px solid #3B82F6' : '3px solid transparent',
                }}
              ...
              >
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: regionCode === r.code ? '#93C5FD' : '#E2E8F0' }}>{r.name}</div>
                  <div style={{ fontSize: 10, color: '#64748B' }}>{r.currency} · ~{r.avg_shipping_days}d shipping</div>
                </div>
                {regionCode === r.code && <span style={{ marginLeft: 'auto', color: '#3B82F6', fontSize: 14 }}>&#10003;</span>}
              </button>
            ))}
          </div>
        )}
