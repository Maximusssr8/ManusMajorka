...
export function ProductFilterSidebar({ open, onToggle, categories, onFiltersChange }: Props) {
  ...
  const checkPill = (label: string, checked: boolean, onClick: () => void) => (
    <label key={label} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', cursor: 'pointer' }}>
      <input type="checkbox" checked={checked} onChange={onClick} style={{ accentColor: '#4f8ef7', width: 14, height: 14, cursor: 'pointer' }} />
      ...
      <div style={{
        ...
        <div style={{ width: 228, padding: '60px 16px 24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
            ...
            <button onClick={reset} style={{ fontSize: 11, color: '#4f8ef7', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Reset</button>
          </div>
          ...
          {(['all', 'growing', 'rapid', 'declining'] as const).map(g => (
            <label key={g} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', cursor: 'pointer' }}>
              <input type="radio" checked={filters.growthFilter === g} onChange={() => update({ growthFilter: g })} style={{ accentColor: '#4f8ef7', cursor: 'pointer' }} />
          ...
          <div style={{ padding: '0 2px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#9CA3AF', marginTop: 4 }}>
              <span>50</span>
              <span style={{ fontWeight: 700, color: '#4f8ef7' }}>{filters.scoreMin}+</span>
              <span>95</span>
            </div>
          </div>
