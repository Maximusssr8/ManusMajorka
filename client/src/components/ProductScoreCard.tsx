...
@keyframes score-pulse {
  0%, 100% { text-shadow: 0 0 20px rgba(79,142,247,0.3); }
  50%       { text-shadow: 0 0 40px rgba(79,142,247,0.6); }
}
`;
...
function getVerdict(score: number): { label: string; color: string; dot: string } {
  ...
  if (score >= 50) return { label: 'Test First', color: '#4f8ef7', dot: '🟡' };
    ...
    return (
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          <RefreshCw size={12} color="#4f8ef7" style={{ animation: 'spin 1s linear infinite' }} />
          <span style={{ fontSize: 11, color: '#4f8ef7', fontWeight: 700, fontFamily: "'Syne', sans-serif", textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            Scoring Opportunity...
          </span>
        </div>
        {[0, 1, 2, 3, 4].map(i => (
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: 10, fontWeight: 700, color: '#4f8ef7', fontFamily: "'Syne', sans-serif", textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 3 }}>
            AI Opportunity Score
          </div>
          {score.productName && (
            <div style={{ fontSize: 13, fontWeight: 700, color: '#F8FAFC', fontFamily: "'Syne', sans-serif" }}>
              {score.productName}
            </div>
          )}
        </div>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{
            ...
            fontSize: 36, color: '#4f8ef7', lineHeight: 1,
