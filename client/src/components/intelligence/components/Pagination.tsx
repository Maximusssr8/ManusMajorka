  ...
  return (
    <div className="flex items-center justify-between px-6 py-4" style={{ borderTop: '1px solid rgba(255,255,255,0.05)', background: '#0a0a0a' }}>
      ...
      <div className="flex items-center gap-1">
        ...
        <div className="flex items-center gap-0.5 mx-1">
          {getPageNumbers().map((p, i) =>
            ...
            <button key={p} onClick={() => onPageChange(p as number)} className={`${btnBase} font-medium`} style={currentPage === p ? { background: '#3B82F6', color: 'white', border: '1px solid #3B82F6' } : { ...btnStyle, color: 'rgba(255,255,255,0.45)' }}>{p}</button>
          )}
        </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={handleJump} className="text-[12px] text-blue-400 hover:text-blue-300 font-medium transition-colors">Go</button>
      </div>
    </div>
  );
}
