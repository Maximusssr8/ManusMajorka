      ...
      <div style={{ background: atLimit ? '#FEF2F2' : '#F9FAFB', border: `1px solid ${atLimit ? '#FECACA' : '#E5E7EB'}`, borderRadius: 10, padding: '10px 16px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
            ...
            {atLimit && (
              <button onClick={() => setShowUpgrade(true)} style={{ fontSize: 11, fontWeight: 700, color: '#4f8ef7', background: 'rgba(79,142,247,0.08)', border: '1px solid #C7D2FE', borderRadius: 6, padding: '3px 10px', cursor: 'pointer' }}>
                Upgrade →
              </button>
            )}
          </div>
