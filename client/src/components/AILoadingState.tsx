...
export function AILoadingState({ messages = DEFAULT_MESSAGES, shape = "text" }: AILoadingStateProps) {
      ...
      <div className="w-full max-w-md mb-8">
        ...
        {shape === "table" && (
          <div className="space-y-2">
            <div className="rounded-lg animate-pulse" style={{ height: 32, background: "rgba(212,175,55,0.06)" }} />
            {[0, 1, 2, 3].map(i => (
              <div
                key={i}
                className="rounded-lg animate-pulse"
                style={{ height: 24, background: "rgba(255,255,255,0.03)", animationDelay: `${i * 0.15}s` }}
              />
            ))}
          </div>
        )}
