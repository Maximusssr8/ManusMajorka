...
export default function ProductInput({ onComplete, session, initialProduct, initialNiche, initialPrice }: {
  ...
  return (
    <div>
      ...
      <p style={{ color: '#94A3B8', marginBottom: 32, fontSize: 15 }}>Tell us about your product and we'll build the complete launch blueprint.</p>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#94A3B8', marginBottom: 6 }}>Product Name *</label>
        ...
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#94A3B8', marginBottom: 6 }}>Description <span style={{ color: '#9CA3AF' }}>(optional but improves quality)</span></label>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#94A3B8', marginBottom: 6 }}>Niche</label>
            <select value={niche} onChange={e => setNiche(e.target.value)} style={{ ...inputStyle, cursor: 'pointer' }}>
              {NICHES.map(n => <option key={n} value={n}>{n}</option>)}
            </select>
          </div>
          <div>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#94A3B8', marginBottom: 6 }}>Price Point (AUD)</label>
        ...
        <button onClick={handleGenerate} disabled={loading} style={{
          ...
          background: loading ? 'rgba(59,130,246,0.3)' : gold,
