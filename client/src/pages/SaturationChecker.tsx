...
function renderMarkdown(md: string): string {
  let html = md
    // headings
    .replace(/^## (.+)$/gm, '<h2 style="font-family:Syne,sans-serif;font-size:1.25rem;font-weight:700;color:#f5f5f5;margin:1.5rem 0 0.75rem">$1</h2>')
    .replace(/^### (.+)$/gm, '<h3 style="font-family:Syne,sans-serif;font-size:1.1rem;font-weight:600;color:#f5f5f5;margin:1.25rem 0 0.5rem">$1</h3>')
    // bold
    .replace(/\*\*(.+?)\*\*/g, '<strong style="color:#f5f5f5;font-weight:600">$1</strong>')
    // bullet lists
    .replace(/^- (.+)$/gm, '<li style="margin-left:1.25rem;padding:0.15rem 0;color:#a1a1aa;list-style-type:disc">$1</li>')
    // tables
    .replace(/^\|(.+)\|$/gm, (_match, inner: string) => {
      const cells = inner.split("|").map((c: string) => c.trim());
      const row = cells.map((c: string) => `<td style="padding:0.5rem 0.75rem;border-bottom:1px solid rgba(255,255,255,0.06);color:#a1a1aa">${c}</td>`).join("");
      return `<tr>${row}</tr>`;
    })
    // wrap consecutive <tr> in table
    .replace(/((?:<tr>.*<\/tr>\n?)+)/g, '<table style="width:100%;border-collapse:collapse;margin:0.75rem 0;font-size:0.875rem">$1</table>')
    // separator rows in tables (remove them)
    .replace(/<tr><td[^>]*>[-:]+<\/td>(?:<td[^>]*>[-:]+<\/td>)*<\/tr>/g, "")
    // paragraphs — wrap loose lines
    .replace(/^(?!<[hluot])(.*\S.*)$/gm, '<p style="color:#a1a1aa;line-height:1.7;margin:0.4rem 0">$1</p>');
...
const LEVEL_CONFIG: Record<SaturationLevel, { color: string; label: string; desc: string }> = {
  ...
  MEDIUM: { color: "#f59e0b", label: "Medium", desc: "Moderate competition — differentiation needed" },
  HIGH: { color: "#f97316", label: "High", desc: "High competition — strong USP required" },
  OVERSATURATED: { color: "#ef4444", label: "Oversaturated", desc: "Market is crowded — consider alternatives" },
};
...
export default function SaturationChecker() {
  ...
  function saveRecent(name: string) {
    const updated = [name, ...recentChecks.filter((c) => c.toLowerCase() !== name.toLowerCase())].slice(0, 5);
      ...
      <div style={containerStyle}>
        ...
        {error && (
          ...
          >
            ...
            <button
              onClick={handleCheck}
              style={{ ...goldButton, marginTop: "1rem", fontSize: "0.85rem", padding: "0.5rem 1.5rem" }}
            >
              Retry
            </button>
          </div>
