...
export default function PublicProfitCalculator() {
  ...
  const calc = useMemo(() => {
    ...
    const projections = [100, 500, 1000].map((orders) => ({
      orders, revenue: revenue * orders, costs: totalCosts * orders, profit: netMarginAmt * orders,
    }));
    ...
    if (netMarginPct > 30) {
      ...
      verdictReason = "Strong margins above 30%. This product has healthy room for scaling ad spend and absorbing returns.";
    } else if (netMarginPct >= 15) {
      verdictColor = "#eab308"; verdictLabel = "Proceed with Caution";
      verdictReason = "Margins between 15-30% can work but leave limited room for error. Consider optimising costs.";
    } else {
    ...
    return {
      revenue, totalCosts, grossMarginAmt, grossMarginPct, netMarginAmt, netMarginPct,
      roas, breakEvenRoas, projections, afterpayEligible, verdictColor, verdictLabel, verdictReason,
    };
  ...
  const labelStyle: React.CSSProperties = {
    fontFamily: dm, fontSize: 13, color: "#a1a1aa", marginBottom: 8, display: "block",
  };

  const numberInputStyle: React.CSSProperties = {
    fontFamily: dm, background: "#15151a", border: "1px solid rgba(255,255,255,0.08)",
    borderRadius: 8, color: C.text, padding: "8px 12px", width: 90, fontSize: 14, outline: "none",
  };

  const InputGroup = ({ label, value, onChange, min, max, step, prefix = "$" }: {
    ...
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        />
        <div style={{ display: "flex", alignItems: "center", gap: 4, fontFamily: dm, color: "#a1a1aa", fontSize: 14 }}>
  ...
  const ResultCard = ({ label, value, sub }: { label: string; value: string; sub?: string }) => (
    <div style={{ ...cardStyle, padding: "20px 24px" }}>
      ...
      <div style={{ fontFamily: syne, fontSize: 28, fontWeight: 700, color: C.text, lineHeight: 1.2 }}>{value}</div>
      {sub && <div style={{ fontFamily: dm, fontSize: 13, color: "#a1a1aa", marginTop: 4 }}>{sub}</div>}
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: dm }}>
      ...
      <div style={{ textAlign: "center", padding: "60px 24px 40px" }}>
        ...
        <h1 style={{ fontFamily: syne, fontWeight: 900, fontSize: "clamp(28px, 5vw, 44px)", color: C.text, marginBottom: 12 }}>
          Dropshipping Profit Calculator
        </h1>
        <p style={{ color: C.secondary, fontSize: 17, maxWidth: 600, margin: "0 auto" }}>
          Calculate your real margins for the Australian market. Includes platform fees, shipping, GST, and ad spend.
        </p>
      </div>

      {/* Calculator */}
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px 40px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32 }} className="profit-calc-grid">
        {/* Inputs */}
        <div>
          <div style={cardStyle}>
            <h2 style={{ fontFamily: syne, fontSize: 18, fontWeight: 600, color: C.text, margin: "0 0 24px" }}>Product & Costs</h2>
            <InputGroup label="Product Cost (AUD)" value={productCost} onChange={setProductCost} min={0} max={200} step={0.5} />
            <InputGroup label="Shipping to AU (AUD)" value={shipping} onChange={setShipping} min={0} max={50} step={0.5} />

            <div style={{ marginBottom: 20 }}>
              <label style={labelStyle}>Platform Fees</label>
              <select value={platformKey} onChange={(e) => setPlatformKey(e.target.value)}
                style={{ width: "100%", fontFamily: dm, background: "#15151a", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, color: C.text, padding: "10px 12px", fontSize: 14, outline: "none", cursor: "pointer" }}>
                {PLATFORM_OPTIONS.map((o) => (
                  <option key={o.label} value={o.label}>{o.label}{o.value > 0 ? ` (${o.value}%)` : ""}</option>
                ))}
              </select>
              {platformKey === "Custom" && (
            ...
            <InputGroup label="Ad Spend per Order (AUD)" value={adSpend} onChange={setAdSpend} min={0} max={50} step={0.5} />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ ...cardStyle, borderColor: calc.verdictColor, borderWidth: 2 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
              <div style={{ width: 12, height: 12, borderRadius: "50%", background: calc.verdictColor, boxShadow: `0 0 12px ${calc.verdictColor}55` }} />
              <span style={{ fontFamily: syne, fontSize: 22, fontWeight: 700, color: calc.verdictColor }}>{calc.verdictLabel}</span>
            </div>
            <p style={{ fontFamily: dm, fontSize: 14, color: "#a1a1aa", margin: 0, lineHeight: 1.6 }}>{calc.verdictReason}</p>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <ResultCard label="Gross Margin" value={fmt(calc.grossMarginAmt)} sub={pct(calc.grossMarginPct)} />
            <ResultCard label="Net Margin" value={fmt(calc.netMarginAmt)} sub={pct(calc.netMarginPct)} />
          </div>

          {/* Projections */}
          <div style={cardStyle}>
            <h3 style={{ fontFamily: syne, fontSize: 16, fontWeight: 600, color: C.text, margin: "0 0 16px" }}>Monthly Projections</h3>
            <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: dm, fontSize: 14 }}>
              <thead>
                <tr>
                  {["Orders", "Revenue", "Costs", "Profit"].map((h) => (
                    <th key={h} style={{ textAlign: "left", padding: "10px 12px", color: C.muted, fontWeight: 500, fontSize: 12, textTransform: "uppercase", borderBottom: `1px solid ${C.border}` }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {calc.projections.map((row) => (
                  <tr key={row.orders}>
                    <td style={{ padding: 12, color: C.text, borderBottom: `1px solid rgba(255,255,255,0.04)` }}>{row.orders.toLocaleString()}</td>
                    <td style={{ padding: 12, color: C.text, borderBottom: `1px solid rgba(255,255,255,0.04)` }}>{fmt(row.revenue)}</td>
                    <td style={{ padding: 12, color: "#a1a1aa", borderBottom: `1px solid rgba(255,255,255,0.04)` }}>{fmt(row.costs)}</td>
                    <td style={{ padding: 12, color: row.profit >= 0 ? C.green : "#ef4444", fontWeight: 600, borderBottom: `1px solid rgba(255,255,255,0.04)` }}>{fmt(row.profit)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section style={{
        ...
        <h2 style={{ fontFamily: syne, fontWeight: 800, fontSize: 28, color: C.text, marginBottom: 12 }}>
          Want 50+ more AU-specific tools?
        </h2>
        <p style={{ color: C.secondary, fontSize: 16, maxWidth: 500, margin: "0 auto 28px", lineHeight: 1.6 }}>
          Product research, ad copy, brand identity, email sequences, competitor analysis — all built for Australian sellers.
        </p>
        <Link href="/app" style={{
          display: "inline-flex", alignItems: "center", gap: 8,
          background: `linear-gradient(135deg, ${C.gold}, #b8941f)`,
          color: "#000", borderRadius: 12, padding: "16px 36px",
          fontFamily: syne, fontWeight: 800, fontSize: 17, textDecoration: "none",
