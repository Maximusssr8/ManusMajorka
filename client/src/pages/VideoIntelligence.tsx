      ...
      <div style={{ background: '#0d0d10', borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '16px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button onClick={triggerRefresh} disabled={refreshing}
            style={{ height: 36, padding: '0 16px', background: refreshing ? '#9CA3AF' : '#3B82F6', color: 'white', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: refreshing ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div style={{ padding: '12px 28px', background: '#05070F', borderBottom: '1px solid #F3F4F6' }}>
        <div style={{ maxWidth: 680, display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button onClick={() => searchInput.trim() && runSearch(searchInput.trim())}
            style={{ height: 40, padding: '0 18px', background: '#3B82F6', color: 'white', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' as const }}>
            Search
          </button>
        </div>
      ...
      <div style={{ padding: '20px 28px', maxWidth: 1360, margin: '0 auto' }}>
        ...
        {searchLoading && (
          <div style={{ marginBottom: 24 }}>
            <div style={{ fontSize: 13, color: '#3B82F6', fontWeight: 600, marginBottom: 12 }}>Searching TikTok for "{searchInput}"…</div>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
              ...
              <a key={i} href={v.url || v.creatorProfileUrl || `https://www.tiktok.com/search?q=${encodeURIComponent(v.title || v.product_mentioned || '')}`} target="_blank" rel="noopener noreferrer"
                ...
                {v.thumbnail && <img src={v.thumbnail} alt="" loading="lazy" style={{ width: '100%', height: 140, objectFit: 'cover' as const }} onError={e => ((e.target as HTMLImageElement).style.display = 'none')} />}
                <div style={{ padding: '10px 12px' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' as const }}>
                    <span style={{ fontSize: 11, color: '#3B82F6', fontWeight: 600 }}>▶ {(v.playCount ?? 0) >= 1000000 ? `${((v.playCount ?? 0)/1000000).toFixed(1)}M` : (v.playCount ?? 0) >= 1000 ? `${((v.playCount ?? 0)/1000).toFixed(0)}K` : v.playCount ?? 0}</span>
          ...
          {NICHES.map(n => {
            ...
            return (
              <button key={n} onClick={() => setSelectedNiche(selectedNiche === n ? '' : n)}
                style={{ height: 30, padding: '0 14px', background: selectedNiche === n ? '#3B82F6' : 'white', color: selectedNiche === n ? 'white' : count === 0 ? '#D1D5DB' : '#374151', border: `1px solid ${selectedNiche === n ? '#3B82F6' : '#E5E7EB'}`, borderRadius: 20, fontSize: 11, fontWeight: 600, cursor: 'pointer', textTransform: 'capitalize' as const }}>
                {n} {count > 0 && `(${count})`}
              </button>
            );
          })}
          ...
          <div>
            {loading ? (
              ...
              <div style={{ textAlign: 'center' as const, padding: '60px 20px', maxWidth: 480, margin: '0 auto' }}>
                ...
                {selectedNiche ? (
                  <button onClick={() => setSelectedNiche('')}
                    style={{ height: 44, padding: '0 28px', background: '#3B82F6', color: 'white', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                    Show all videos
                  </button>
                ) : (
                  <button onClick={triggerRefresh} disabled={refreshing}
                  ...
                  <div key={i} style={{ padding: '10px 14px', borderBottom: '1px solid #F9FAFB' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingLeft: 26 }}>
                      ...
                      <button onClick={() => copyText(v.hook_text!, `hook-${i}`)}
                        style={{ fontSize: 10, fontWeight: 700, color: copied === `hook-${i}` ? '#059669' : '#3B82F6', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
          ...
          <div style={{ background: '#0d0d10', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden' }}>
            ...
            <div style={{ padding: 14 }}>
              ...
              <button onClick={generateContent} disabled={aiLoading}
                style={{ width: '100%', height: 38, background: aiLoading ? '#9CA3AF' : 'linear-gradient(135deg, #3B82F6, #8B5CF6)', color: 'white', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: aiLoading ? 'not-allowed' : 'pointer', marginBottom: aiOutput ? 10 : 0 }}>
