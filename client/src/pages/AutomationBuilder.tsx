...
interface AutoResult { summary: string; automations: Automation[]; totalTimeSaved: string; }
    ...
    <div className="h-full flex" style={{ background: "#080a0e", color: "#f0ede8", fontFamily: "DM Sans, sans-serif" }}>
      <div className="w-80 flex-shrink-0 overflow-y-auto border-r p-5 space-y-4" style={{ borderColor: "rgba(255,255,255,0.07)", scrollbarWidth: "thin" }}>
        <div className="flex items-center gap-2.5 mb-1">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "rgba(255,180,0,0.12)", border: "1px solid rgba(255,180,0,0.3)" }}>
            <Zap size={15} style={{ color: "#ffb400" }} />
          </div>
          <div>
            <div className="text-sm font-black" style={{ fontFamily: "Syne, sans-serif" }}>Automation Builder</div>
            <div className="text-xs" style={{ color: "rgba(240,237,232,0.35)" }}>Workflows · Triggers · Tools</div>
          </div>
        </div>

        {[
          { label: "Business Type", value: businessType, set: setBusinessType, placeholder: "e.g. Dropshipping pet products", req: true, area: false },
          { label: "Pain Points", value: painPoints, set: setPainPoints, placeholder: "e.g. Manual order processing, slow customer replies", req: false, area: true },
          { label: "Current Tools", value: currentTools, set: setCurrentTools, placeholder: "e.g. Shopify, Klaviyo, Gorgias", req: false, area: false },
          { label: "Monthly Budget ($)", value: budget, set: setBudget, placeholder: "e.g. 200", req: false, area: false },
        ...
        <button onClick={handleGenerate} disabled={generating}
          ...
          style={{ background: generating ? "rgba(255,180,0,0.25)" : "linear-gradient(135deg, #ffb400, #f0c040)", color: "#080a0e", fontFamily: "Syne, sans-serif", boxShadow: generating ? "none" : "0 4px 20px rgba(255,180,0,0.3)", cursor: generating ? "not-allowed" : "pointer" }}>
          {generating ? <><Loader2 size={14} className="animate-spin" /> Building…</> : <><Zap size={14} /> Build Automations</>}
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6" style={{ scrollbarWidth: "thin" }}>
        {result ? (
          <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div className="rounded-xl p-4" style={{ background: "rgba(255,180,0,0.06)", border: "1px solid rgba(255,180,0,0.2)" }}>
            ...
            {result.automations.map((auto, i) => (
              <div key={i} className="rounded-xl overflow-hidden" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
                  <div>
                    <div className="text-sm font-black" style={{ fontFamily: "Syne, sans-serif" }}>{auto.name}</div>
                    <div className="text-xs mt-0.5" style={{ color: "rgba(240,237,232,0.4)" }}>{auto.description}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs px-2 py-0.5 rounded-full font-bold" style={{ background: `${diffColor(auto.difficulty)}12`, color: diffColor(auto.difficulty) }}>{auto.difficulty}</span>
                    <span className="text-xs font-mono" style={{ color: "#2dca72" }}>{auto.timeSaved}</span>
                  </div>
                </div>
                    ...
                    {auto.steps.map((step, j) => (
                      <div key={j} className="flex items-center gap-2">
                        <div className="rounded-lg px-3 py-2" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
                          <div className="text-xs font-bold" style={{ color: "#ffb400" }}>{step.trigger}</div>
                          <div className="text-xs mt-0.5" style={{ color: "rgba(240,237,232,0.5)" }}>{step.action}</div>
                          <div className="text-xs mt-0.5 flex items-center gap-1" style={{ color: "rgba(240,237,232,0.3)" }}>
                            <Settings size={8} /> {step.tool}
                          </div>
                        </div>
                        {j < auto.steps.length - 1 && <ArrowRight size={14} style={{ color: "rgba(255,255,255,0.15)" }} />}
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="px-4 pb-3 flex flex-wrap gap-1.5">
                  {auto.tools.map((tool, j) => (
                    <span key={j} className="text-xs px-2 py-0.5 rounded-full" style={{ background: "rgba(255,180,0,0.08)", border: "1px solid rgba(255,180,0,0.15)", color: "rgba(255,200,80,0.8)" }}>{tool}</span>
                  ))}
                </div>
              </div>
            ))}
          ...
          <div className="flex flex-col items-center justify-center h-full gap-4">
            {generating ? (
              <div className="text-center">
                <Loader2 size={32} className="animate-spin mx-auto mb-4" style={{ color: "#ffb400" }} />
                <div className="text-sm font-bold" style={{ fontFamily: "Syne, sans-serif" }}>Building your automations…</div>
              </div>
            ) : (
              <>
                <div className="text-5xl">⚡</div>
                <div className="text-center">
                  <div className="text-base font-black mb-2" style={{ fontFamily: "Syne, sans-serif" }}>Automation Builder</div>
                  <div className="text-xs max-w-xs leading-relaxed" style={{ color: "rgba(240,237,232,0.35)" }}>
                    Describe your business and pain points to get custom automation workflows with tools, triggers, and time savings.
                  </div>
                </div>
              </>
            )}
