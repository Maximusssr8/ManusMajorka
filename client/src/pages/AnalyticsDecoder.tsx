...
export default function AnalyticsDecoder() {
  ...
  const handleGenerate = useCallback(async () => {
    if (!revenue && !sessions) { toast.error("Enter at least revenue or sessions."); return; }
    setGenerating(true);
    setResult(null);
    sendMessage({
  ...
  }, [revenue, sessions, convRate, aov, adSpend, returnRate, bounceRate, cartAbandonment, sendMessage]);
    ...
    <div className="h-full flex" style={{ background: "#080a0e", color: "#f0ede8", fontFamily: "DM Sans, sans-serif" }}>
      ...
      <div className="w-80 flex-shrink-0 overflow-y-auto border-r p-5 space-y-4" style={{ borderColor: "rgba(255,255,255,0.07)", scrollbarWidth: "thin" }}>
        <div className="flex items-center gap-2.5 mb-1">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "rgba(244,114,182,0.12)", border: "1px solid rgba(244,114,182,0.3)" }}>
            <BarChart3 size={15} style={{ color: "#f472b6" }} />
          </div>
          <div>
            <div className="text-sm font-black" style={{ fontFamily: "Syne, sans-serif" }}>Analytics Decoder</div>
            <div className="text-xs" style={{ color: "rgba(240,237,232,0.35)" }}>KPIs · Benchmarks · Actions</div>
          </div>
        </div>

        {[
          { label: "Monthly Revenue ($)", value: revenue, set: setRevenue, placeholder: "e.g. 12500" },
          { label: "Monthly Sessions", value: sessions, set: setSessions, placeholder: "e.g. 15000" },
          { label: "Conversion Rate (%)", value: convRate, set: setConvRate, placeholder: "e.g. 2.1" },
          { label: "Average Order Value ($)", value: aov, set: setAov, placeholder: "e.g. 65" },
          { label: "Monthly Ad Spend ($)", value: adSpend, set: setAdSpend, placeholder: "e.g. 3000" },
          { label: "Return Rate (%)", value: returnRate, set: setReturnRate, placeholder: "e.g. 5" },
          { label: "Bounce Rate (%)", value: bounceRate, set: setBounceRate, placeholder: "e.g. 45" },
          { label: "Cart Abandonment (%)", value: cartAbandonment, set: setCartAbandonment, placeholder: "e.g. 70" },
        ...
        <button
          ...
          style={{
            background: generating ? "rgba(244,114,182,0.25)" : "linear-gradient(135deg, #f472b6, #ec4899)",
            color: "#fff", fontFamily: "Syne, sans-serif",
            boxShadow: generating ? "none" : "0 4px 20px rgba(244,114,182,0.3)",
            cursor: generating ? "not-allowed" : "pointer",
          }}
        >
          {generating ? <><Loader2 size={14} className="animate-spin" /> Analyzing…</> : <><BarChart3 size={14} /> Decode Analytics</>}
        </button>
      </div>
          ...
          <div className="max-w-3xl mx-auto space-y-6">
              ...
              {result.kpis.map((kpi, i) => (
                <div key={i} className="rounded-xl p-4" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)" }}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs" style={{ color: "rgba(240,237,232,0.4)" }}>{kpi.name}</span>
                    <div className="flex items-center gap-1 text-xs font-semibold" style={{ color: kpi.trend === "up" ? "#2dca72" : kpi.trend === "down" ? "#ff6b6b" : "#f0c040" }}>
                      {kpi.trend === "up" ? <TrendingUp size={11} /> : kpi.trend === "down" ? <TrendingDown size={11} /> : null}
                      {kpi.change}
                    </div>
                  </div>
                  <div className="text-lg font-black mb-1" style={{ fontFamily: "Syne, sans-serif" }}>{kpi.value}</div>
                  <div className="text-xs" style={{ color: "rgba(240,237,232,0.4)" }}>{kpi.insight}</div>
                </div>
              ))}
            </div>
            ...
            <div className="rounded-xl overflow-hidden" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)" }}>
              <div className="px-4 py-3 border-b" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
                <div className="text-xs font-black uppercase tracking-widest" style={{ color: "#f472b6", fontFamily: "Syne, sans-serif" }}>Key Insights</div>
              </div>
              {result.insights.map((insight, i) => (
                <div key={i} className="flex items-start gap-3 px-4 py-3" style={{ borderBottom: i < result.insights.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                  <span className="text-xs mt-0.5" style={{ color: "#f472b6" }}>💡</span>
                  <span className="text-xs leading-relaxed" style={{ color: "rgba(240,237,232,0.65)" }}>{insight}</span>
                </div>
              ))}
            </div>

            {/* Actions */}
            <div className="rounded-xl overflow-hidden" style={{ background: "rgba(45,202,114,0.04)", border: "1px solid rgba(45,202,114,0.15)" }}>
              <div className="px-4 py-3 border-b" style={{ borderColor: "rgba(45,202,114,0.1)" }}>
                <div className="text-xs font-black uppercase tracking-widest" style={{ color: "#2dca72", fontFamily: "Syne, sans-serif" }}>Recommended Actions</div>
              </div>
              {result.actions.map((action, i) => (
                <div key={i} className="flex items-start gap-3 px-4 py-3" style={{ borderBottom: i < result.actions.length - 1 ? "1px solid rgba(45,202,114,0.08)" : "none" }}>
                  <ArrowRight size={12} className="mt-0.5 flex-shrink-0" style={{ color: "#2dca72" }} />
                  <span className="text-xs leading-relaxed" style={{ color: "rgba(240,237,232,0.65)" }}>{action}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col items-center justify-center h-full gap-4">
            {generating ? (
              <div className="text-center">
                <Loader2 size={32} className="animate-spin mx-auto mb-4" style={{ color: "#f472b6" }} />
                <div className="text-sm font-bold" style={{ fontFamily: "Syne, sans-serif" }}>Decoding your analytics…</div>
                <div className="text-xs mt-1" style={{ color: "rgba(240,237,232,0.35)" }}>Comparing against industry benchmarks</div>
              </div>
            ) : (
              <>
                <div className="text-5xl">📈</div>
                <div className="text-center">
                  <div className="text-base font-black mb-2" style={{ fontFamily: "Syne, sans-serif" }}>Analytics Decoder</div>
                  <div className="text-xs max-w-xs leading-relaxed" style={{ color: "rgba(240,237,232,0.35)" }}>
                    Enter your store metrics to get KPI analysis with industry benchmarks, key insights, and recommended actions.
                  </div>
                </div>
              </>
            )}
