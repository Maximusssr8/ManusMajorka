  ...
  return (
    <button
      ...
      style={{
        ...
        background: copied ? 'rgba(245,158,11,0.12)' : '#F9FAFB',
        border: `1px solid ${copied ? 'rgba(245,158,11,0.3)' : '#F0F0F0'}`,
...
function LaunchKitContent() {
  ...
  useEffect(() => {
    ...
    style.textContent = `@media print {
      ...
      #launchkit-printable { background: #05070F; color: black; padding: 24px; }
      button, .no-print { display: none !important; }
    }`;
    document.head.appendChild(style);
    return () => {
      document.getElementById('launchkit-print')?.remove();
    };
  }, []);

  const updateStep = useCallback((id: string, patch: Partial<LaunchKitStep>) => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    const aiSteps: Array<{ id: string; systemPrompt: string }> = [
      {
        id: 'brand-dna',
        systemPrompt: injectProductIntelligence(PROMPTS['brand-dna'], activeProduct as any),
      },
      {
        id: 'product-copy',
        systemPrompt: injectProductIntelligence(PROMPTS['product-copy'], activeProduct as any),
      },
      {
        id: 'website-html',
        systemPrompt: injectProductIntelligence(PROMPTS['website-html'], activeProduct as any),
      },
      {
        id: 'ad-copy',
        systemPrompt: injectProductIntelligence(
          PROMPTS['ad-copy'] + productName,
          activeProduct as any
        ),
      },
      {
        id: 'email-sequence',
        systemPrompt: injectProductIntelligence(PROMPTS['email-sequence'], activeProduct as any),
      },
    ];
  ...
  return (
    <div className="flex h-full overflow-hidden" style={{ background: '#05070F' }}>
      {/* ── Left Panel ── */}
      <div
        ...
        <div className="flex-1 px-4 py-4 space-y-1.5">
          {steps.map((step, i) => (
            <div
              ...
              style={{
                background: step.status === 'running' ? 'rgba(245,158,11,0.07)' : 'transparent',
        ...
        <div className="px-4 pb-6 pt-2">
          <button
            ...
            style={{
              ...
              color: !activeProduct || isRunning ? 'rgba(245,158,11,0.4)' : '#0a0a0a',
              border: 'none',
      ...
      <div className="flex-1 flex flex-col overflow-hidden" style={{ minWidth: 0 }}>
        ...
        <div
          ...
          style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', background: '#05070F' }}
        >
          <div>
            <h2
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-xs" style={{ color: '#94A3B8' }}>
                        <span style={{ color: '#ef4444', fontWeight: 700 }}>{step.label}</span>{' '}
