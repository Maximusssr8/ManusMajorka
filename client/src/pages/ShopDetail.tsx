...
const C = { bg: '#FAFAFA', surface: '#FFFFFF', border: '#E5E7EB', gold: '#d4af37', text: '#0A0A0A', muted: 'rgba(55,65,81,0.5)' };
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
            {[
              ...
              { label: 'Growth Rate', value: `${shop.growth_rate_pct > 0 ? '+' : ''}${shop.growth_rate_pct}%`, color: shop.growth_rate_pct >= 0 ? '#d4af37' : '#ef4444' },
                ...
                <div>
                  ...
                  {analysis.why_succeeding.map((point, i) => (
                    <div key={i} style={{ fontSize: 13, color: C.text, padding: '6px 0', borderBottom: i < analysis.why_succeeding.length - 1 ? `1px solid ${C.border}` : 'none', display: 'flex', gap: 8 }}>
                      <span style={{ color: '#d4af37', flexShrink: 0 }}>✓</span> {point}
                    </div>
                  ))}
                </div>
                <div>
                ...
                {shop.similar_shops.map(s => (
                  <div key={s.id} onClick={() => navigate(`/app/shops/${s.id}`)}
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ fontSize: 11, color: s.growth_rate_pct >= 0 ? '#d4af37' : '#ef4444' }}>{s.growth_rate_pct > 0 ? '+' : ''}{s.growth_rate_pct}%</div>
                    </div>
                  </div>
                ))}
              </div>
