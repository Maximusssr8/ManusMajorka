  ...
  return (
    <div style={{ minHeight: '100vh', background: '#05070F', fontFamily: dm }}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(250,250,250,0.95)', backdropFilter: 'blur(16px)', borderBottom: '1px solid #ECECEC', height: 64, display: 'flex', alignItems: 'center', padding: '0 24px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            ...
            <Link href="/sign-up" style={{ background: '#4f8ef7', color: 'white', borderRadius: 8, padding: '7px 16px', fontWeight: 600, fontSize: 14, textDecoration: 'none' }}>
              Get Started →
            </Link>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '64px 24px 48px', textAlign: 'center' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(79,142,247,0.08)', borderRadius: 20, padding: '4px 14px', marginBottom: 20 }}>
          <span style={{ fontSize: 12, fontWeight: 700, color: '#4f8ef7', letterSpacing: '0.06em', textTransform: 'uppercase' as const }}>Blog</span>
          <span style={{ fontSize: 11, background: 'rgba(79,142,247,0.15)', color: '#6ba3ff', borderRadius: 6, padding: '2px 8px', fontWeight: 700 }}>Content Coming Soon</span>
        </div>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '0 24px 80px' }}>
          ...
          {POSTS.map((post, idx) => (
            <article key={post.slug} style={{ background: '#0d0d10', borderRadius: 16, border: `1px solid ${idx === 0 ? '#C7D2FE' : '#E5E7EB'}`, padding: '28px 32px', transition: 'box-shadow 200ms, transform 200ms', position: 'relative' as const }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, flexWrap: 'wrap' as const }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: '#4f8ef7', background: 'rgba(79,142,247,0.08)', borderRadius: 6, padding: '2px 8px', letterSpacing: '0.04em', textTransform: 'uppercase' as const }}>{post.category}</span>
        ...
        <div style={{ textAlign: 'center', marginTop: 48, padding: '32px', background: '#0d0d10', borderRadius: 16, border: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <Link href="/sign-up" style={{ background: '#4f8ef7', color: 'white', borderRadius: 8, padding: '10px 24px', fontWeight: 600, fontSize: 14, textDecoration: 'none', display: 'inline-block' }}>
            Join Majorka →
          </Link>
        </div>
      </div>
