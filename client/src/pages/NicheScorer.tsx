...
const VERDICT_CONFIG = {
  "Strong Go": { color: "#2dca72", bg: "rgba(45,202,114,0.1)", border: "rgba(45,202,114,0.25)", emoji: "🟢" },
  "Proceed with Caution": { color: "#d4af37", bg: "rgba(212,175,55,0.1)", border: "rgba(212,175,55,0.25)", emoji: "🟡" },
  "Avoid": { color: "#e05c7a", bg: "rgba(224,92,122,0.1)", border: "rgba(224,92,122,0.25)", emoji: "🔴" },
};
...
const EXAMPLE_NICHES = ["Posture correctors", "Weighted blankets", "LED grow lights", "Bamboo kitchenware", "Dog anxiety products"];
    ...
    <div className="h-full flex flex-col" style={{ background: "#080a0e", color: "#f0ede8", fontFamily: "DM Sans, sans-serif" }}>
      <div className="flex items-center gap-3 px-5 py-3 border-b flex-shrink-0" style={{ borderColor: "rgba(255,255,255,0.07)", background: "#0c0e12" }}>
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-black leading-tight" style={{ fontFamily: "Syne, sans-serif" }}>Niche Scorer</div>
          <div className="text-xs" style={{ color: "rgba(240,237,232,0.35)" }}>6-dimension scoring · Go/No-go verdict · Product recommendations</div>
        </div>
        {result && (
      ...
      <div className="flex-1 overflow-hidden flex">
        <div className="w-72 flex-shrink-0 overflow-y-auto border-r p-4 space-y-4" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-xs font-semibold mb-1.5" style={{ color: "rgba(240,237,232,0.6)", fontFamily: "Syne, sans-serif" }}>Additional Context</label>
          ...
          <div>
            <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: "rgba(240,237,232,0.3)", fontFamily: "Syne, sans-serif" }}>Examples</div>
            <div className="flex flex-wrap gap-1.5">
              {EXAMPLE_NICHES.map(n => (
                <button key={n} onClick={() => setNiche(n)}
                  className="text-xs px-2.5 py-1 rounded-lg transition-all"
                  style={{ background: niche === n ? "rgba(212,175,55,0.1)" : "rgba(255,255,255,0.04)", border: `1px solid ${niche === n ? "rgba(212,175,55,0.3)" : "rgba(255,255,255,0.07)"}`, color: niche === n ? "#d4af37" : "rgba(240,237,232,0.45)", cursor: "pointer" }}>
                  {n}
                </button>
              ))}
            </div>
          ...
          <button onClick={handleGenerate} disabled={isLoading}
            ...
            {isLoading ? <><Loader2 size={14} className="animate-spin" /> Scoring…</> : <><BarChart2 size={14} /> Score This Niche</>}
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {isLoading && !result && (
            <div className="h-full flex flex-col items-center justify-center gap-4">
              ...
              <div className="text-center">
                <div className="text-sm font-black mb-1" style={{ fontFamily: "Syne, sans-serif" }}>Scoring {niche}…</div>
                <div className="text-xs" style={{ color: "rgba(240,237,232,0.35)" }}>Evaluating across 6 dimensions</div>
              </div>
            </div>
          )}
          ...
          {result && vc && (
            <div className="space-y-4 max-w-2xl">
              ...
              <div className="p-5 rounded-2xl text-center" style={{ background: vc.bg, border: `1.5px solid ${vc.border}` }}>
                ...
                <div className="text-sm mt-3 leading-relaxed" style={{ color: "rgba(240,237,232,0.75)" }}>{result.summary}</div>
              </div>

              {/* Score breakdown */}
              <div className="p-4 rounded-2xl" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <div className="text-xs font-bold uppercase tracking-widest mb-4" style={{ color: "rgba(240,237,232,0.4)", fontFamily: "Syne, sans-serif" }}>Score Breakdown</div>
                <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="p-3 rounded-xl text-center" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
                  <div className="text-xs font-bold mb-1" style={{ color: "rgba(240,237,232,0.35)", fontFamily: "Syne, sans-serif" }}>Time to Sale</div>
                  <div className="text-xs font-black" style={{ color: "#d4af37", fontFamily: "Syne, sans-serif" }}>{result.timeToFirstSale}</div>
                </div>
                <div className="p-3 rounded-xl text-center" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
                  <div className="text-xs font-bold mb-1" style={{ color: "rgba(240,237,232,0.35)", fontFamily: "Syne, sans-serif" }}>Ideal Customer</div>
                  <div className="text-xs" style={{ color: "rgba(240,237,232,0.6)" }}>{result.idealCustomer.slice(0, 40)}…</div>
                </div>
              </div>

              {/* Top products */}
              <div className="p-4 rounded-2xl" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
                <div className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "rgba(240,237,232,0.4)", fontFamily: "Syne, sans-serif" }}>Top Product Ideas</div>
                <div className="flex flex-wrap gap-2">
                  {result.topProducts.map((p, i) => (
                    <span key={i} className="text-xs px-3 py-1.5 rounded-lg" style={{ background: "rgba(212,175,55,0.08)", border: "1px solid rgba(212,175,55,0.15)", color: "rgba(240,237,232,0.7)" }}>
                      {p}
                    </span>
                  ))}
                </div>
              ...
              <div className="p-4 rounded-2xl" style={{ background: "rgba(212,175,55,0.04)", border: "1px solid rgba(212,175,55,0.12)" }}>
                <div className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "#d4af37", fontFamily: "Syne, sans-serif" }}>Next Steps</div>
                <div className="space-y-2">
                  {result.nextSteps.map((step, i) => (
                    <div key={i} className="flex items-start gap-2 text-xs" style={{ color: "rgba(240,237,232,0.75)" }}>
                      <span className="w-4 h-4 rounded flex items-center justify-center flex-shrink-0 mt-0.5 font-black text-xs" style={{ background: "rgba(212,175,55,0.15)", color: "#d4af37", fontFamily: "Syne, sans-serif" }}>{i + 1}</span>
                      {step}
                    </div>
                  ))}
                </div>
          ...
          {!isLoading && !result && (
            <div className="h-full flex flex-col items-center justify-center gap-4 p-8">
              ...
              <div className="text-center">
                <div className="text-base font-black mb-2" style={{ fontFamily: "Syne, sans-serif" }}>Get a data-driven go/no-go verdict</div>
                <div className="text-xs max-w-xs leading-relaxed" style={{ color: "rgba(240,237,232,0.35)" }}>
                  Enter any niche to get scored across 6 dimensions: demand, competition, margins, trends, entry barriers, and scalability.
                </div>
              </div>
            </div>
          )}
