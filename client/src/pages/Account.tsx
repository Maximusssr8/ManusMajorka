...
function StatusBadge({ status }: { status: string | undefined }) {
  ...
  if (status === "cancelled")
    return (
      <span
        className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold"
        style={{ background: "rgba(255,165,0,0.1)", border: "1px solid rgba(255,165,0,0.25)", color: "#ffa500", fontFamily: "Syne, sans-serif" }}
      >
        Cancelled
      </span>
    );
  return (
    <span
      className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold"
      style={{ background: "rgba(255,80,80,0.1)", border: "1px solid rgba(255,80,80,0.25)", color: "#ff5050", fontFamily: "Syne, sans-serif" }}
    >
      Expired
    </span>
  );
      ...
      >
        <div className="container flex items-center justify-between h-14">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-black text-sm" style={{ fontFamily: "Syne, sans-serif" }}>Majorka</span>
          </div>
          <Button
            variant="ghost"
            size="sm"
      ...
      <main className="relative z-10 container py-12 max-w-2xl">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="font-bold text-base" style={{ fontFamily: "Syne, sans-serif" }}>{user?.name}</div>
            ...
            {subscription ? (
              <>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs text-muted-foreground mb-1 uppercase tracking-wider font-semibold" style={{ fontFamily: "Syne, sans-serif" }}>
                      Plan
                    </p>
                ...
                {isActive && (
                  <div
                    ...
                    style={{ background: "rgba(45,202,114,0.06)", border: "1px solid rgba(45,202,114,0.15)", color: "rgba(45,202,114,0.9)" }}
                  >
                ...
                {subscription.status === "cancelled" && (
                  <div
                    className="flex items-center gap-2 p-3 rounded-xl text-sm"
                    style={{ background: "rgba(255,165,0,0.06)", border: "1px solid rgba(255,165,0,0.15)", color: "rgba(255,165,0,0.9)" }}
                  >
                    <XCircle className="w-4 h-4 flex-shrink-0" />
                    Your subscription is cancelled. Access ends {periodEnd?.toLocaleDateString() ?? "soon"}.
                  </div>
                )}
              </>
            ) : (
              <div className="text-center py-4">
                ...
                <p className="text-xs text-muted-foreground">Subscribe to unlock the full Majorka AI OS.</p>
              </div>
            )}
          </div>
        ...
        >
          <p className="flex items-start gap-2">
            ...
            Subscriptions are billed monthly. Cancel anytime — you'll retain access until the end of your current billing period. For billing support, contact us at{" "}
            <a href="mailto:support@majorka.com" className="underline hover:text-foreground transition-colors">
              support@majorka.com
            </a>
          </p>
        </div>
