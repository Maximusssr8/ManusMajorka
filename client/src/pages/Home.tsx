...
const FAQ_DATA: { q: string; a: string }[] = [
  { q: 'How is Majorka different from Minea, KaloData, Ecomhunt, or AutoDS?', a: "Minea and KaloData are research-only — you still have to bolt on Shopify, an ad generator, and a store builder yourself. AutoDS is fulfillment, not intelligence. Majorka is the only AU-first platform that ships all of it — AliExpress + TikTok Shop data, ad brief generator, store builder, creator matrix, and a first-class public API — under one subscription. Minea and KaloData ship zero public API; Majorka's is documented at /app/api-docs." },
  { q: 'Is the product data actually accurate, or is it estimated?', a: 'Every row in the product database is tied to a real AliExpress or CJ Dropshipping product ID. Order counts, prices, and ratings come from the official affiliate APIs — not scraped estimates. The catalog refreshes every 6 hours (see /v1/stats/overview for a live updatedAt timestamp). Winning scores are computed server-side from the raw signals so you can always verify the math.' },
  ...
  { q: 'Which markets are supported?', a: 'Australia, United States, United Kingdom, Canada, New Zealand, Germany, and Singapore. Each market has localised pricing, supplier recommendations, tax calculations, and regional ad benchmarks built in. AUD is the default — you can switch currencies in Settings.' },
  { q: 'What does "Early Access" actually mean? Is it a beta?', a: "Early Access means the product is fully live, production-stable, and billed on Stripe — but features are shipping weekly rather than monthly. You lock in founding-member pricing for life, you get direct Slack access to the team, and you can request features that ship to you first. There is no feature gating based on Early Access — you get everything in your plan tier." },
  { q: 'Can I cancel anytime? What happens to my data?', a: "Yes. No contracts, no lock-ins. Cancel before your next billing date and you won't be charged. Every saved product, store blueprint, ad brief, and API key is exportable as CSV from Settings → Export. Exports remain downloadable for 30 days after cancellation. If you come back within 90 days we restore your full workspace with one click." },
  { q: 'Will my AI briefs and API calls eat a hidden usage bill?', a: "No hidden bills. Builder plan includes 1,000 API requests/day and 50 AI ad briefs/month. Scale plan includes 10,000 API requests/day and 500 AI briefs/month. When you approach 80% of any limit, the app warns you. You cannot be charged overage fees — the feature is rate-limited instead of billed-through. The usage meter is visible on every page." },
  { q: 'How fast is support if something breaks?', a: "Builder plan: email within 24 hours (usually much faster). Scale plan: priority email + Slack Connect channel within 4 hours Sydney business hours, 12 hours out-of-hours. Production outages page the founders directly. For anything critical, email support@majorka.io — it goes to a human, not a bot." },
  { q: "Is my data secure and GDPR-compliant?", a: "Data is stored on Supabase (Postgres) in Sydney ap-southeast-2 with row-level security. API keys are stored as sha256 hashes — even a database breach cannot leak live keys. GDPR and Australian Privacy Act compliant. You can request a full data export or deletion at any time via support@majorka.io and we process within 7 days." },
  { q: "Do you have a free trial or money-back guarantee?", a: "Yes to both. The Academy tab is free to everyone, including the first track of product research lessons — no card needed to start. The Builder and Scale plans come with a 30-day money-back guarantee. Cancel inside 30 days and email us and we refund the whole month, no questions asked. Most operators decide whether Majorka is a fit inside the first 48 hours." },
  { q: "Does Majorka really integrate with Shopify, Meta, and TikTok?", a: "Yes — Shopify has one-click OAuth in Store Builder (publishing + product sync); Meta Ad Library data is surfaced inside Ads Studio for ad spy; TikTok Shop data flows in via the Apify + DataHub pipeline every 3 hours. Native Meta Ads Manager posting is on the public roadmap for Q3 2026 but you can already export ad briefs as CSVs that drop directly into Meta's bulk upload." },
  { q: 'What if I want to build my own tools on top of Majorka data?', a: "You can. Every operator gets access to the Developer API at /app/api-keys — create a key, read /app/api-docs, and hit the same data the dashboard uses from curl, n8n, Zapier, or your own scripts. No other ecommerce intelligence platform ships a real public API. This alone is worth the subscription if you run multi-store operations." },
];
