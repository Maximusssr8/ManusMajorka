...
const NICHE_COLORS: Record<string, string> = {
  beauty: '#EC4899', tech: '#3B82F6', fitness: '#10B981', home: '#F59E0B',
  'home decor': '#F59E0B', fashion: '#8B5CF6', pet: '#F97316', 'pet care': '#F97316',
  kitchen: '#3B82F6', health: '#10B981', outdoor: '#6B7280', 'tech accessories': '#3B82F6',
  ecommerce: '#3B82F6', dropshipping: '#8B5CF6', baby: '#EC4899', general: '#6B7280',
};
...
export default function CreatorIntelligence() {
  ...
  const renderCreatorCard = (c: Creator, opts?: { featured?: boolean; tierIndex?: number }) => {
    ...
    return (
      <div
        ...
        style={{
          background: isSelected ? 'rgba(59,130,246,0.15)' : isHovered ? 'rgba(255,255,255,0.07)' : 'rgba(255,255,255,0.04)',
          border: `1px solid ${isSelected ? 'rgba(59,130,246,0.5)' : isHovered ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.08)'}`,
          borderRadius: 16,
          padding,
          cursor: 'pointer',
  ...
  const errorFallback = (
    <div className="flex flex-col items-center justify-center h-full py-24 gap-6" style={{ background: '#080808' }}>
      ...
      <button onClick={() => window.location.reload()} className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-lg text-[13px] font-medium transition-colors">
        Try again
      </button>
    </div>
  );
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
            ...
            {isScale && (
              <button onClick={triggerRefresh} disabled={refreshing}
                style={{ padding: '6px 14px', background: refreshing ? 'rgba(255,255,255,0.1)' : 'rgba(59,130,246,0.2)', color: 'white', border: '1px solid rgba(59,130,246,0.3)', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: refreshing ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <div>
          {loading ? (
            ...
            <div style={{ background: '#111114', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 60, textAlign: 'center' as const }}>
              ...
              <a href="/app/videos" style={{ display: 'inline-block', height: 38, lineHeight: '38px', padding: '0 20px', background: '#3B82F6', color: 'white', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', textDecoration: 'none' }}>
                Browse Video Intel →
              </a>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', margin: 0, lineHeight: 1.5 }}>
                  💡 Creators with 50K–500K followers have <span style={{ color: '#3B82F6', fontWeight: 700 }}>3x better response rates</span>
                </p>
              </div>
            ...
            <div style={{ background: '#111114', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 20 }}>
              <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
                ...
                <div>
                  ...
                  <a href={getTikTokUrl(selected)} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: '#3B82F6', textDecoration: 'none' }}>{selected.handle} ↗</a>
                </div>
              </div>
              {(() => {
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' as const }}>
                    {selected.promoting_products.map((p, i) => (
                      <span key={i} style={{ fontSize: 10, color: '#93C5FD', background: 'rgba(59,130,246,0.12)', padding: '2px 8px', borderRadius: 10 }}>{p}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              <button onClick={() => generateOutreach(selected)} disabled={outreachLoading}
                style={{ width: '100%', height: 38, background: 'linear-gradient(135deg, #7C3AED, #3B82F6)', color: 'white', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', marginBottom: 12, opacity: outreachLoading ? 0.7 : 1 }}>
              ...
              {outreach && (
                <div style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: 8, padding: 12 }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: '#3B82F6', marginBottom: 6 }}>AI-Generated Pitch:</div>
                  ...
                  <button onClick={() => navigator.clipboard.writeText(outreach)}
                    style={{ marginTop: 8, fontSize: 10, color: '#3B82F6', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, padding: 0 }}>
                    Copy message →
                  </button>
                </div>
              )}
