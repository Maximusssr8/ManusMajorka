                ...
                <tr key={u.id} style={{ borderBottom: `1px solid ${C.border}` }}
                  ...
                  <td style={{ padding: '10px 12px' }}>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      ...
                      >
                        {['free','pro','builder','scale'].map(p => <option key={p} value={p} style={{ background: '#0a0b0d' }}>{p}</option>)}
                      </select>
                      <button
                        onClick={() => updatePlan(u.id, 'pro', 'active')}
                        disabled={updatingId === u.id}
                        style={{ padding: '4px 8px', background: C.goldBg, border: `1px solid rgba(212,175,55,0.2)`, borderRadius: 5, color: C.gold, fontSize: 11, cursor: 'pointer' }}
                      >Grant Pro</button>
                      <button
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => setShowAddForm(!showAddForm)} style={{ padding: '7px 14px', background: C.goldBg, border: `1px solid rgba(212,175,55,0.2)`, borderRadius: 7, color: C.gold, fontSize: 12, cursor: 'pointer' }}>+ Add Product</button>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: 16, marginBottom: 16 }}>
            ...
            {([['name','Product Name'],['niche','Niche'],['estimated_retail_aud','Price AUD'],['estimated_margin_pct','Margin %'],['trend_score','Trend Score (0-100)'],['dropship_viability_score','Viability (0-10)']] as [string,string][]).map(([k, label]) => (
              <div key={k}>
                ...
                <input value={(form as any)[k]} onChange={e => setForm(f => ({ ...f, [k]: e.target.value }))}
                  style={{ width: '100%', padding: '6px 10px', background: '#0a0b0f', border: `1px solid ${C.border}`, borderRadius: 6, color: C.text, fontSize: 12, boxSizing: 'border-box' as const }} />
              </div>
            ))}
          </div>
          <div style={{ marginBottom: 10 }}>
            ...
            <input value={form.trend_reason} onChange={e => setForm(f => ({ ...f, trend_reason: e.target.value }))}
              style={{ width: '100%', padding: '6px 10px', background: '#0a0b0f', border: `1px solid ${C.border}`, borderRadius: 6, color: C.text, fontSize: 12 }} />
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
        <button onClick={() => setShowAdd(!showAdd)} style={{ padding: '7px 14px', background: C.goldBg, border: `1px solid rgba(212,175,55,0.2)`, borderRadius: 7, color: C.gold, fontSize: 12, cursor: 'pointer' }}>+ Manual Subscription</button>
      </div>
            ...
            <div key={k} style={{ flex: k === 'email' ? 2 : 1 }}>
              ...
              {k === 'plan' ? (
                <select value={addForm.plan} onChange={e => setAddForm(f => ({ ...f, plan: e.target.value }))}
                  style={{ width: '100%', padding: '7px 10px', background: '#0a0b0f', border: `1px solid ${C.border}`, borderRadius: 6, color: C.text, fontSize: 12 }}>
                  {['pro','builder','scale','free'].map(p => <option key={p} value={p} style={{ background: '#0a0b0d' }}>{p}</option>)}
                </select>
              ) : (
