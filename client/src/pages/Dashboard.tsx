  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ fontFamily: "'Syne', sans-serif", color: '#4f8ef7', letterSpacing: '-0.02em', fontSize: 28, fontWeight: 700 }}
      >
        {displayValue ??
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <a href="/app/winning-products" style={{ fontSize: 12, color: '#4f8ef7', textDecoration: 'none' }}>View all →</a>
      </div>
      ...
      {!userNiche && (
        <div style={{ marginTop: 12, padding: '10px 14px', background: 'rgba(79,142,247,0.05)', border: '1px solid rgba(79,142,247,0.15)', borderRadius: 10 }}>
          <span style={{ fontSize: 12, color: '#94A3B8' }}>
            💡 Set your niche in <a href="/app/settings" style={{ color: '#4f8ef7' }}>Settings</a> to see personalised product recommendations
          </span>
        </div>
      )}
    </div>
...
const CAT_CFG: Record<string, { bg: string; text: string }> = {
  ...
  Supplements: { bg: 'rgba(79,142,247,0.15)',  text: '#6ba3ff' },
};
  ...
  return (
    <div style={{ marginBottom: 32, fontFamily: UI }}>
      ...
      {isDemo && !bannerDismissed && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14, padding: '6px 12px', background: 'rgba(79,142,247,0.06)', border: '1px solid rgba(79,142,247,0.14)', borderRadius: 8 }}>
          <span style={{ fontSize: 11, color: 'rgba(79,142,247,0.75)', fontWeight: 500, flex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 3, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: 3 }}>
          {RANGE_OPTIONS.map((r) => (
            <button key={r.key} onClick={() => setRange(r.key)} style={{
              ...
              background: range === r.key ? '#4f8ef7' : 'transparent',
        ...
        {kpiCards.map((k) => (
          <div key={k.label} className="mkr-kpi-card">
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 9 }}>
              <k.icon size={12} style={{ color: '#6ba3ff' }} />
      ...
      <div style={{ background: '#111114', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '18px 14px 10px', marginBottom: 12 }}>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <AreaChart data={chartData} margin={{ top: 4, right: 4, left: -10, bottom: 0 }}>
            ...
            <Tooltip content={<SalesTooltip />} cursor={{ stroke: 'rgba(79,142,247,0.2)', strokeWidth: 1 }} />
            <Area type="monotone" dataKey="revenue" stroke="#4f8ef7" strokeWidth={1.8} fill="url(#goldGrad)" dot={false} activeDot={{ r: 3, fill: '#4f8ef7', strokeWidth: 0 }} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="mkr-table-wrap" style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 20px 0' }}>
          ...
          <a href="/app/winning-products" style={{ fontSize: 12, color: '#6ba3ff', textDecoration: 'none', fontWeight: 500 }}>View all →</a>
        </div>
          ...
          <table className="mkr-table">
            ...
            <tbody>
              {DEMO_PRODUCTS.map((p, i) => {
                ...
                const cc = isDark
                  ? { bg: 'rgba(79,142,247,0.15)', text: '#6ba3ff' }
                  : { bg: 'rgba(79,142,247,0.15)', text: '#6ba3ff' };
                return (
                  <tr key={p.rank}>
    ...
    <div style={{ marginBottom: 20 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#4f8ef7', display: 'inline-block', marginRight: 8 }} />
          <span style={{ fontFamily: "'Syne', sans-serif", fontSize: 17, fontWeight: 700, color: '#FFFFFF' }}>This Week&apos;s Leaders</span>
        </div>
        <button onClick={() => setLocation('/app/intelligence')} style={{ fontSize: 13, color: '#4f8ef7', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 500 }}>View all →</button>
      </div>
            ...
            return (
              <div key={p.id} onClick={() => setLocation('/app/intelligence')} style={{ flexShrink: 0, width: 160, background: '#111114', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 12, cursor: 'pointer' }}>
                ...
                {p.image_url ? (
                  ...
                  <div style={{ width: 40, height: 40, borderRadius: 6, background: 'rgba(79,142,247,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, color: '#6ba3ff', marginBottom: 8 }}>
              ...
              {leaderboard.map((p, i) => {
                ...
                return (
                  <tr key={p.id} style={{ borderBottom: i < leaderboard.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none', cursor: 'pointer', transition: 'background 120ms' }}
                    ...
                    <td style={{ padding: '10px 16px' }}>
                      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                        {p.image_url ? (
                          ...
                          <div style={{ width: 36, height: 36, borderRadius: 6, background: 'rgba(79,142,247,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: '#6ba3ff', flexShrink: 0 }}>
                            {(p.product_title || 'P').charAt(0)}
                          </div>
                        )}
                    ...
                    <td style={{ padding: '10px 16px' }}>
                      <button onClick={e => { e.stopPropagation(); setLocation('/app/intelligence'); }} style={{ background: 'rgba(79,142,247,0.2)', color: '#6ba3ff', borderRadius: 6, padding: '4px 8px', fontSize: 12, border: 'none', cursor: 'pointer', fontWeight: 600 }}>→</button>
                    </td>
                  </tr>
                );
              })}
...
function DailyBrief() {
  ...
  return (
    <div className="mkr-card" style={{ padding: '18px 20px', marginBottom: 20 }}>
      ...
      {loading ? (
        <div style={{ height: 56, background: 'rgba(79,142,247,0.06)', borderRadius: 8, animation: 'shimmer 1.8s ease-in-out infinite' }} />
      ) : (
        <>
          <p style={{ fontSize: 13, color: '#CBD5E1', lineHeight: 1.75, whiteSpace: 'pre-wrap' as const, margin: 0 }}>{brief}</p>
          <button
            onClick={() => setLocation('/app/intelligence')}
            style={{ marginTop: 12, fontSize: 12, color: '#6ba3ff', background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontWeight: 500 }}
            onMouseEnter={e => (e.currentTarget.style.color = '#A5B4FC')}
            onMouseLeave={e => (e.currentTarget.style.color = '#6ba3ff')}
          >
            Find these products →
          </button>
        </>
    ...
    <div style={{
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 12, color: '#6ba3ff', background: 'rgba(79,142,247,0.2)', border: '1px solid rgba(79,142,247,0.3)', borderRadius: 20, padding: '2px 10px' }}>
      ...
      <div style={{ padding: '0 20px', marginBottom: checklistOpen ? 12 : 0 }}>
        <div style={{ width: '100%', height: 4, background: 'rgba(255,255,255,0.1)', borderRadius: 99 }}>
          <div style={{ height: 4, background: '#4f8ef7', borderRadius: 99, width: `${(completedSteps.size / GETTING_STARTED_STEPS.length) * 100}%`, transition: 'width 300ms' }} />
        </div>
      </div>
      {checklistOpen && (
          ...
          {GETTING_STARTED_STEPS.map(step => (
            <div
              ...
              style={{
                ...
                background: completedSteps.has(step.id) ? 'rgba(79,142,247,0.06)' : 'rgba(255,255,255,0.03)',
                cursor: 'pointer', transition: 'background 0.15s',
              }}
              onMouseEnter={e => { if (!completedSteps.has(step.id)) e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; }}
              onMouseLeave={e => { e.currentTarget.style.background = completedSteps.has(step.id) ? 'rgba(79,142,247,0.06)' : 'rgba(255,255,255,0.03)'; }}
            >
              <div style={{
                width: 20, height: 20, borderRadius: '50%',
                border: completedSteps.has(step.id) ? '2px solid #4f8ef7' : '2px solid rgba(255,255,255,0.3)',
                background: completedSteps.has(step.id) ? '#4f8ef7' : 'transparent',
    ...
    <div className="page-transition" style={{ minHeight: '100vh', background: '#0a0a0a', padding: '0' }}>
      ...
      <div style={{ background: '#0a0a0a', borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '24px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <div style={{ marginBottom: 4 }}>
            ...
            <span style={{ fontFamily: "'Syne', system-ui, sans-serif", fontSize: 24, fontWeight: 700, color: '#4f8ef7', letterSpacing: '-0.02em' }}>{firstName}</span>
          </div>
          <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif", fontSize: 13, color: '#52525b' }}>{formatDate()}{planLabel ? ` · ${planLabel}` : ''}</div>
        </div>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          {!isPaid && (
            <button onClick={() => setLocation('/pricing')} style={{ height: 38, padding: '0 18px', background: '#4f8ef7', color: 'white', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'transform 150ms' }}
      ...
      <div style={{ padding: 'clamp(16px, 3vw, 32px)', maxWidth: 1400, margin: '0 auto' }}>
        ...
        <div className="stats-grid-responsive" style={{ gap: 16, marginBottom: 20 }}>
          {([
            ...
            <div key={i} onClick={() => card.path && setLocation(card.path)} className="mkr-kpi-card"
              onMouseEnter={e => { e.currentTarget.style.boxShadow = '0 4px 16px rgba(79,142,247,0.08)'; e.currentTarget.style.borderColor = 'rgba(79,142,247,0.3)'; }}
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                ...
                <div style={{ width: 34, height: 34, borderRadius: 9, background: 'rgba(79,142,247,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <card.icon size={16} color="#6ba3ff" />
                </div>
              </div>
        ...
        <div className="chart-insight-grid">
          ...
          <div className="mkr-card" style={{ padding: '24px 28px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div>
                <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 17, color: '#F1F5F9' }}>Est. Market Opportunity</div>
              ...
              <div style={{ textAlign: 'right' as const }}>
                <span style={{ fontSize: 22, fontWeight: 800, fontFamily: 'Syne, sans-serif', color: '#22C55E' }}>{loading ? '—' : estimatedWeeklyOpp === 0 ? 'N/A' : fmtWeeklyDisplay}</span>
            ...
            <ResponsiveContainer width="100%" height={140}>
              <AreaChart data={chartData} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                ...
                <Area type="monotone" dataKey="rev" stroke="#4f8ef7" strokeWidth={2.5} fill="url(#gold-grad)" dot={{ fill: '#4f8ef7', r: 3 }} activeDot={{ r: 5 }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="mkr-card" style={{ padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
              <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 17, color: '#F1F5F9' }}>Your Workflow</div>
              <span style={{ fontSize: 11, color: '#6ba3ff', fontWeight: 600, background: 'rgba(79,142,247,0.15)', padding: '2px 8px', borderRadius: 20, border: '1px solid rgba(79,142,247,0.25)' }}>Click any step →</span>
            </div>
            <div style={{ fontSize: 12, color: '#94A3B8', marginBottom: 16 }}>The fastest path from zero to first sale</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {([
                { step: '1', icon: Search, label: 'Find a winning product', sub: 'Filter by niche, margin & trend', path: '/app/product-intelligence', done: false, color: '#4f8ef7' },
                { step: '2', icon: BarChart2, label: 'Run the numbers', sub: 'Check margins before buying stock', path: '/app/profit', done: false, color: '#4f8ef7' },
                ...
                { step: '4', icon: Megaphone, label: 'Generate your ad creative', sub: 'Maya writes Meta + TikTok ads', path: '/app/ads-studio', done: false, color: '#4f8ef7' },
                ...
                <button key={i} onClick={() => setLocation(s.path)}
                  ...
                  onMouseEnter={e => { e.currentTarget.style.background = 'rgba(79,142,247,0.08)'; e.currentTarget.style.borderColor = `${s.color}60`; e.currentTarget.style.borderLeftColor = s.color; }}
        ...
        <div className="products-ai-grid">
          ...
          <div style={{ background: '#111114', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 17, color: '#F1F5F9' }}>Top Products Today</div>
              <button onClick={() => setLocation('/app/intelligence')} style={{ fontSize: 13, color: '#6ba3ff', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 500 }}>View all &rarr;</button>
            </div>
            {loading ? (
              ...
              <div>
                {products.length === 0 ? (
                  <div style={{ padding: '32px 24px', textAlign: 'center', color: '#9CA3AF', fontSize: 13 }}>
                    ...
                    <a href="/app/intelligence" style={{ color: '#6ba3ff', fontWeight: 600, textDecoration: 'none', fontSize: 13 }}>Browse Product Intelligence →</a>
                  </div>
                ) : null}
                {(products.slice(0, 5)).map((p: any, i: number) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ width: 40, height: 40, borderRadius: 8, background: 'rgba(79,142,247,0.15)', display: p.image_url ? 'none' : 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, color: '#6ba3ff', flexShrink: 0 }}>
                      ...
                      <div style={{ textAlign: 'right' as const, flexShrink: 0 }}>
                        ...
                        <div style={{ marginTop: 4 }}>
                          <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: '50px', ...(score >= 80 ? { background: '#4f8ef7', color: 'white' } : score >= 65 ? { background: 'transparent', border: '1.5px solid #4f8ef7', color: '#6ba3ff' } : { background: 'rgba(255,255,255,0.06)', color: '#9CA3AF' }) }}>{score}</span>
                        </div>
                      </div>
                    </div>
                  );
          ...
          {products[0] && (
            <div style={{ background: 'rgba(79,142,247,0.12)', border: '1px solid rgba(79,142,247,0.3)', borderRadius: 14, padding: '24px', color: 'white', display: 'flex', flexDirection: 'column' }}>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: '#6ba3ff', marginBottom: 12 }}>🏆 Top Product Right Now</div>
              ...
              <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 16, lineHeight: 1.4, marginBottom: 8, color: 'white', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as const }}>{products[0].product_title}</div>
              ...
              <button onClick={() => setLocation('/app/intelligence')} style={{ marginTop: 16, padding: '10px 16px', background: 'rgba(79,142,247,0.2)', color: '#6ba3ff', border: '1px solid rgba(79,142,247,0.3)', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'background 150ms' }}
                onMouseEnter={e => (e.currentTarget.style.background = 'rgba(79,142,247,0.3)')}
                onMouseLeave={e => (e.currentTarget.style.background = 'rgba(79,142,247,0.2)')}
        ...
        <div style={{ background: '#111114', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 17, color: '#F1F5F9' }}>Your Tools</div>
