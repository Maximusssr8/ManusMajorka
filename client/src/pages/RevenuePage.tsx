...
function Tip({ active, payload }: { active?: boolean; payload?: { payload: Pt }[] }) {
  ...
  return (
    <div style={{ background: '#0A0F1E', border: '1px solid rgba(212,175,55,0.4)', borderRadius: 12, padding: '12px 18px', boxShadow: '0 12px 32px rgba(0,0,0,0.6)' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #030608 0%, #080C18 35%, #0a0a0a 100%)', color: '#fff', fontFamily: geist, paddingBottom: 'calc(env(safe-area-inset-bottom,0px) + 80px)' }}>
      ...
      <style>{`
        ...
        .rev-card:hover { border-color:rgba(212,175,55,.4) !important; transform:translateY(-3px); box-shadow:0 12px 40px rgba(0,0,0,.5) !important; }
        .rev-order   { transition: background 150ms, border-color 150ms, transform 120ms; }
        .rev-order:hover { background:rgba(255,255,255,.05) !important; border-color:rgba(212,175,55,.25) !important; transform:translateX(2px); }
      ...
      <div style={{
        ...
        <div style={{ position:'absolute',bottom:-60,left:-30, width:400,height:400, background:'radial-gradient(circle,rgba(212,175,55,.13) 0%,transparent 60%)', pointerEvents:'none', animation:'orb2 11s ease-in-out infinite' }} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 32 : 56, alignItems: 'center' }}>
            ...
            <div style={{ animation: 'fadeUp .6s ease both' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 28, flexWrap: 'wrap' as const }}>
                <div style={{ display:'flex',alignItems:'center',gap:7, background:'rgba(212,175,55,.14)', border:'1px solid rgba(212,175,55,.3)', borderRadius:999, padding:'6px 14px' }}>
              ...
              {isMarketing && (
                <div style={{ display:'flex',alignItems:'center',gap:10,marginBottom:28,flexWrap:'wrap' as const }}>
                  ...
                  <div style={{ display:'flex',alignItems:'center',gap:6, background:'rgba(212,175,55,.1)', border:'1px solid rgba(212,175,55,.2)', borderRadius:999, padding:'7px 14px' }}>
            ...
            <div style={{ ...glass, borderRadius:24, padding: isMobile ? '20px 16px' : '24px 24px', display: isMobile ? 'none' : 'flex', flexDirection:'column' as const, animation:'fadeUp .6s .15s ease both', opacity: 0, animationFillMode: 'forwards' as const }}>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={chartData} margin={{ top:4,right:4,left:-20,bottom:0 }}>
                  ...
                  <Tooltip content={<Tip />} cursor={{ stroke:'rgba(212,175,55,.3)',strokeWidth:1,strokeDasharray:'4 4' }} />
      ...
      {!isMarketing && (
        <div style={{ display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', minHeight: '60vh', gap: 24, textAlign: 'center' as const, padding: '40px 24px' }}>
          ...
          <button
            ...
            style={{ padding: '14px 32px', background: '#d4af37', color: 'white', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: 'pointer' as const, fontFamily: "'Syne',sans-serif" }}
          >
      ...
      {isMarketing && (
        <div style={{ maxWidth:1200,margin:'0 auto',padding: isMobile ? '16px 16px 0' : '24px 52px 0',display:'grid',gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr',gap:16 }}>
          ...
          <div style={{ ...glass,borderRadius:24,padding: isMobile ? '22px 18px' : '28px 26px',boxShadow:'0 4px 32px rgba(0,0,0,.3)' }}>
            <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:22 }}>
              <div style={{ display:'flex',alignItems:'center',gap:9 }}>
                <div style={{ width:34,height:34,borderRadius:10,background:'rgba(212,175,55,.15)',border:'1px solid rgba(212,175,55,.25)',display:'flex',alignItems:'center',justifyContent:'center' }}>
      ...
      <div style={{ maxWidth:1200,margin:'0 auto',padding: isMobile ? '16px 16px 0' : '24px 52px 0' }}>
        ...
        <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16 }}>
          ...
          {isMarketing && (
            <div style={{ display:'flex',gap:10,alignItems:'center' }}>
              ...
              <button onClick={handleShare} style={{ display:'flex',alignItems:'center',gap:6,background:'rgba(212,175,55,.1)',border:'1px solid rgba(212,175,55,.2)',borderRadius:999,padding:'7px 14px',cursor:'pointer',color:I,fontSize:12,fontWeight:700 }}>
        ...
        {orders.length === 0 && (
          <div style={{ ...glass,borderRadius:24,padding:'52px 24px',textAlign:'center' as const,boxShadow:'0 4px 32px rgba(0,0,0,.3)' }}>
            ...
            <button onClick={() => nav('/app/store-builder')} style={{ padding:'11px 22px',borderRadius:12,background:I,color:'#fff',border:'none',fontSize:14,fontWeight:700,cursor:'pointer',fontFamily:brico,boxShadow:'0 4px 16px rgba(212,175,55,.35)' }}>
              Connect Store
            </button>
          </div>
        )}
                  ...
                  <div className="rev-order" style={{
                    ...
                    <div style={{ flex:1,minWidth:0 }}>
                      ...
                      <div style={{ display:'flex',alignItems:'center',gap:8,flexWrap:'wrap' as const }}>
                        ...
                        {o.qty > 1 && <span style={{ fontSize:10,color:I,fontWeight:700,background:'rgba(212,175,55,.1)',borderRadius:999,padding:'1px 7px' }}>×{o.qty}</span>}
                      </div>
                    </div>
                    {/* Amount + status */}
      ...
      <div style={{ maxWidth:1200,margin:'0 auto',padding: isMobile ? '16px 16px 0' : '24px 52px 0',display:'grid',gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr',gap:16 }}>
        ...
        {/* Connect store */}
        <div style={{ background:'linear-gradient(135deg,#0C1528 0%,#180E3A 100%)',border:'1px solid rgba(212,175,55,.22)',borderRadius:24,padding: isMobile ? '24px 20px' : '32px 30px',position:'relative' as const,overflow:'hidden' as const,boxShadow:'0 8px 40px rgba(0,0,0,.4)' }}>
          <div style={{ position:'absolute',top:-40,right:-40,width:160,height:160,background:'radial-gradient(circle,rgba(212,175,55,.2) 0%,transparent 65%)',pointerEvents:'none' }} />
          <div style={{ width:44,height:44,borderRadius:14,background:'rgba(212,175,55,.15)',border:'1px solid rgba(212,175,55,.3)',display:'flex',alignItems:'center',justifyContent:'center',marginBottom:16 }}>
            <Zap size={22} color={I} />
          </div>
          ...
          <button onClick={() => nav('/app/store-builder')} style={{ padding:'13px 24px',borderRadius:14,border:'none',background:`linear-gradient(135deg,${I},${V})`,color:'#fff',fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:brico,display:'flex',alignItems:'center',gap:8,boxShadow:`0 6px 24px rgba(212,175,55,.4)` }}>
