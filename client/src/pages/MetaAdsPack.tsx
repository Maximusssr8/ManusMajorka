...
import { Copy, Send, Loader2, Sparkles, Megaphone } from "lucide-react";
import { toast } from "sonner";
import { DefaultChatTransport } from "ai";

const META_ADS_SYSTEM_PROMPT = `You are an expert Meta Ads strategist specializing in high-converting Facebook and Instagram campaigns for ecommerce.

Your role is to help users create complete Meta Ads launch packs including:
1. Campaign structure (awareness, consideration, conversion)
2. Ad copy variations (3-5 versions per angle)
3. Visual creative briefs
4. Audience targeting recommendations
5. Budget allocation strategy
6. Performance tracking setup

When a user provides their product/service details, ask for:
- Product/service description and price
- Target audience demographics
- Main selling points (top 3)
- Campaign budget
- Previous campaign performance (if any)
- Conversion goal (sales, leads, traffic)

Generate structured, actionable Meta Ads strategies with specific copy, targeting, and creative recommendations.`;
      ...
      prepareSendMessagesRequest({ messages }) {
        return {
          body: {
            ...
            chatId: "meta-ads-pack",
            systemPrompt: META_ADS_SYSTEM_PROMPT,
          },
        };
      },
    }),
      ...
      <div className="flex-shrink-0 border-b p-4" style={{ borderColor: "rgba(255,255,255,0.08)" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Create high-converting Facebook & Instagram campaigns</p>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-4 p-4">
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full text-center py-12">
                <Megaphone className="w-12 h-12 mb-4" style={{ color: "#d4af37", opacity: 0.3 }} />
                <p className="text-sm text-muted-foreground max-w-xs">
                  Tell me about your product, and I'll create a complete Meta Ads campaign strategy for you.
                </p>
              </div>
            )}
              ...
              <div className="flex gap-3 justify-start">
                ...
                <div className="bg-muted rounded-lg px-4 py-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">Generating campaign strategy...</span>
                  </div>
                </div>
              </div>
            )}
