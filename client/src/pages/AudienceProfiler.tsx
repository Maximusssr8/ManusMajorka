    ...
    <div className="rounded-2xl overflow-hidden" style={{ background: c.bg, border: `1.5px solid ${c.border}` }}>
      <div className="p-4">
        <div className="flex items-start justify-between mb-3">
          <div>
            <div className="text-sm font-black" style={{ fontFamily: "Syne, sans-serif", color: "#f0ede8" }}>{persona.name}</div>
            <div className="text-xs mt-0.5" style={{ color: "rgba(240,237,232,0.45)" }}>
              {persona.age} · {persona.gender} · {persona.location}
            </div>
          </div>
          <div className="text-right">
            <div className="text-xs font-semibold" style={{ color: c.accent }}>{persona.occupation}</div>
            <div className="text-xs" style={{ color: "rgba(240,237,232,0.35)" }}>{persona.income}</div>
          </div>
        </div>
        ...
        <div className="flex gap-1 mb-3 p-1 rounded-lg" style={{ background: "rgba(0,0,0,0.2)" }}>
          {(["profile", "psychology", "messaging"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              ...
              style={{ background: tab === t ? c.accent : "transparent", color: tab === t ? "#080a0e" : "rgba(240,237,232,0.4)", cursor: "pointer", fontFamily: "Syne, sans-serif" }}>
              {t}
            </button>
          ))}
        </div>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <div className="p-2.5 rounded-xl" style={{ background: "rgba(0,0,0,0.15)" }}>
                <div className="text-xs font-bold uppercase tracking-wider mb-1.5" style={{ color: "rgba(240,237,232,0.35)", fontFamily: "Syne, sans-serif" }}>Pain Points</div>
                {persona.painPoints.map((p, i) => (
                  <div key={i} className="text-xs flex items-start gap-1.5 mb-1" style={{ color: "rgba(240,237,232,0.7)" }}>
                    <span style={{ color: "#e05c7a", flexShrink: 0 }}>✕</span>{p}
                  </div>
                ))}
              </div>
              <div className="p-2.5 rounded-xl" style={{ background: "rgba(0,0,0,0.15)" }}>
                <div className="text-xs font-bold uppercase tracking-wider mb-1.5" style={{ color: "rgba(240,237,232,0.35)", fontFamily: "Syne, sans-serif" }}>Desires</div>
                {persona.desires.map((d, i) => (
                  <div key={i} className="text-xs flex items-start gap-1.5 mb-1" style={{ color: "rgba(240,237,232,0.7)" }}>
                    <span style={{ color: "#2dca72", flexShrink: 0 }}>✓</span>{d}
                  </div>
                ))}
              </div>
            </div>
            <div className="p-2.5 rounded-xl" style={{ background: "rgba(0,0,0,0.15)" }}>
              <div className="text-xs font-bold uppercase tracking-wider mb-1.5" style={{ color: "rgba(240,237,232,0.35)", fontFamily: "Syne, sans-serif" }}>Audience Size</div>
              <div className="text-sm font-black" style={{ color: c.accent, fontFamily: "Syne, sans-serif" }}>{persona.estimatedSize}</div>
            </div>
          </div>
        )}
        ...
        {tab === "psychology" && (
          <div className="space-y-3">
            <div className="p-2.5 rounded-xl" style={{ background: "rgba(0,0,0,0.15)" }}>
              <div className="text-xs font-bold uppercase tracking-wider mb-1.5" style={{ color: "rgba(240,237,232,0.35)", fontFamily: "Syne, sans-serif" }}>Buying Triggers</div>
            ...
            <div className="p-2.5 rounded-xl" style={{ background: "rgba(0,0,0,0.15)" }}>
              <div className="text-xs font-bold uppercase tracking-wider mb-1.5" style={{ color: "rgba(240,237,232,0.35)", fontFamily: "Syne, sans-serif" }}>Objections to Overcome</div>
              {persona.objections.map((o, i) => (
                <div key={i} className="text-xs flex items-start gap-1.5 mb-1" style={{ color: "rgba(240,237,232,0.7)" }}>
                  <span style={{ color: "#e05c7a", flexShrink: 0 }}>!</span>{o}
                </div>
              ))}
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div className="p-2.5 rounded-xl" style={{ background: "rgba(0,0,0,0.15)" }}>
                <div className="text-xs font-bold uppercase tracking-wider mb-1.5" style={{ color: "rgba(240,237,232,0.35)", fontFamily: "Syne, sans-serif" }}>Channels</div>
                <div className="flex flex-wrap gap-1">
                  {persona.preferredChannels.map((ch, i) => (
                    <span key={i} className="text-xs px-1.5 py-0.5 rounded" style={{ background: `${c.accent}18`, color: c.accent }}>{ch}</span>
                  ))}
                </div>
              </div>
              <div className="p-2.5 rounded-xl" style={{ background: "rgba(0,0,0,0.15)" }}>
                <div className="text-xs font-bold uppercase tracking-wider mb-1.5" style={{ color: "rgba(240,237,232,0.35)", fontFamily: "Syne, sans-serif" }}>Content Types</div>
                <div className="flex flex-wrap gap-1">
                  {persona.contentTypes.map((ct, i) => (
                    <span key={i} className="text-xs px-1.5 py-0.5 rounded" style={{ background: "rgba(255,255,255,0.06)", color: "rgba(240,237,232,0.55)" }}>{ct}</span>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="space-y-3">
            <div className="p-3 rounded-xl" style={{ background: "rgba(0,0,0,0.2)" }}>
              <div className="flex items-center justify-between mb-1.5">
                <div className="text-xs font-bold uppercase tracking-wider" style={{ color: c.accent, fontFamily: "Syne, sans-serif" }}>Messaging Angle</div>
                <CopyBtn text={persona.messagingAngle} />
              </div>
              <div className="text-xs leading-relaxed" style={{ color: "rgba(240,237,232,0.75)" }}>{persona.messagingAngle}</div>
            </div>
            <div className="p-3 rounded-xl" style={{ background: "rgba(212,175,55,0.06)", border: "1px solid rgba(212,175,55,0.15)" }}>
              <div className="flex items-center justify-between mb-1.5">
                <div className="text-xs font-bold uppercase tracking-wider" style={{ color: "#d4af37", fontFamily: "Syne, sans-serif" }}>🎣 Ad Hook</div>
                <CopyBtn text={persona.adHook} />
              </div>
              <div className="text-sm font-semibold leading-relaxed" style={{ color: "#f0ede8" }}>{persona.adHook}</div>
            </div>
          </div>
        )}
      </div>
...
export default function AudienceProfiler() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const searchData = await searchMutation.mutateAsync({
        query: `${product} target audience customer demographics Australia`,
        maxResults: 3,
        searchDepth: "basic",
      });
  ...
  }, [product, pricePoint, sendMessage, searchMutation]);
    ...
    <div className="h-full flex flex-col" style={{ background: "#080a0e", color: "#f0ede8", fontFamily: "DM Sans, sans-serif" }}>
      <div className="flex items-center gap-3 px-5 py-3 border-b flex-shrink-0" style={{ borderColor: "rgba(255,255,255,0.07)", background: "#0c0e12" }}>
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-black leading-tight" style={{ fontFamily: "Syne, sans-serif" }}>Audience Profiler</div>
          <div className="text-xs" style={{ color: "rgba(240,237,232,0.35)" }}>3 buyer personas · Pain points · Ad hooks · Channel strategy</div>
        </div>
        {result && (
      ...
      <div className="flex-1 overflow-hidden flex">
        <div className="w-72 flex-shrink-0 overflow-y-auto border-r p-4 space-y-4" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
          ...
          <button onClick={handleGenerate} disabled={isLoading}
            ...
            {isLoading ? <><Loader2 size={14} className="animate-spin" /> Profiling…</> : <><Users size={14} /> Build Audience Profile</>}
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {isLoading && !result && (
            <div className="h-full flex flex-col items-center justify-center gap-4">
              ...
              <div className="text-center">
                <div className="text-sm font-black mb-1" style={{ fontFamily: "Syne, sans-serif" }}>Building audience profiles…</div>
                <div className="text-xs" style={{ color: "rgba(240,237,232,0.35)" }}>Analysing buyer psychology and demographics</div>
              </div>
            </div>
          )}
          ...
          {result && (
            <div className="space-y-4 max-w-3xl">
              <div className="p-4 rounded-2xl" style={{ background: "rgba(156,95,255,0.05)", border: "1px solid rgba(156,95,255,0.15)" }}>
                <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: "#9c5fff", fontFamily: "Syne, sans-serif" }}>Primary Persona</div>
                <div className="text-sm leading-relaxed" style={{ color: "rgba(240,237,232,0.8)" }}>{result.primaryPersona}</div>
              </div>

              <div className="space-y-4">
                {result.personas.map((p, i) => <PersonaCard key={i} persona={p} index={i} />)}
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 rounded-2xl" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
                  <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: "rgba(240,237,232,0.4)", fontFamily: "Syne, sans-serif" }}>Messaging Framework</div>
                  <div className="text-xs leading-relaxed" style={{ color: "rgba(240,237,232,0.65)" }}>{result.messagingFramework}</div>
                </div>
                <div className="p-4 rounded-2xl" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
                  <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: "rgba(240,237,232,0.4)", fontFamily: "Syne, sans-serif" }}>Channel Strategy</div>
                  <div className="text-xs leading-relaxed" style={{ color: "rgba(240,237,232,0.65)" }}>{result.channelStrategy}</div>
                </div>
              </div>
            </div>
          )}
          ...
          {!isLoading && !result && (
            <div className="h-full flex flex-col items-center justify-center gap-4 p-8">
              ...
              <div className="text-center">
                <div className="text-base font-black mb-2" style={{ fontFamily: "Syne, sans-serif" }}>Know your buyer inside out</div>
                <div className="text-xs max-w-xs leading-relaxed" style={{ color: "rgba(240,237,232,0.35)" }}>
                  Enter a product to get 3 detailed buyer personas with pain points, buying triggers, ad hooks, and channel strategy.
                </div>
              </div>
            </div>
          )}
