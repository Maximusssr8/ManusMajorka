...
const SEED_ADS = [
  ...
  { id:2, platform:'TikTok', product:'LED Desk Lamp RGB', runningDays:23, spendPerDay:187, niche:'Tech',
    hook:"POV: Your home office finally looks like a setup tour 👀", body:"16 million colours, 3 brightness levels, USB-C. £24 anywhere else, $29 AUD here.", cta:"Get Yours", bg:'rgba(79,142,247,0.08)', accent:'#4f8ef7' },
  { id:3, platform:'Instagram', product:'Vitamin C Serum 30ml', runningDays:61, spendPerDay:445, niche:'Beauty',
    hook:"I was embarrassed to go out without makeup. Then I tried this.", body:"Dermatologist-tested 20% Vitamin C. Dark spots visibly reduced in 14 days. Now I leave the house bare-faced.", cta:"Try Risk-Free", bg:'#FDF4FF', accent:'#4f8ef7' },
...
const C = { bg: '#FAFAFA', surface: '#FFFFFF', border: '#E5E7EB', accent: '#4f8ef7', text: '#0A0A0A', muted: '#6B7280' };
      ...
      <div style={{ padding: '0 0 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button onClick={triggerRefresh} disabled={refreshing}
            style={{ height: 36, padding: '0 16px', background: '#4f8ef7', color: 'white', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: refreshing ? 'wait' : 'pointer', opacity: refreshing ? 0.7 : 1, transition: 'all 150ms' }}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr 280px', gap: 16, flex: 1 }}>
        ...
        <div style={{ background: '#0d0d10', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden', height: 'fit-content' }}>
          ...
          {stores.map(s => (
            <button key={s.domain} onClick={() => setSelectedStore(selectedStore === s.domain ? null : s.domain)}
              style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '8px 14px', background: selectedStore === s.domain ? 'rgba(79,142,247,0.08)' : 'white', border: 'none', borderLeft: selectedStore === s.domain ? '3px solid #4f8ef7' : '3px solid transparent', cursor: 'pointer', textAlign: 'left' as const, transition: 'background 120ms' }}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: selectedStore === s.domain ? '#4f8ef7' : '#0A0A0A', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const, direction: 'ltr' as const }}>{s.name}</div>
              </div>
              {(s.product_count || 0) > 0 && (
        ...
        <div style={{ background: '#0d0d10', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid #F3F4F6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {selectedStore && (
              <button onClick={() => setSelectedStore(null)} style={{ fontSize: 11, color: '#4f8ef7', background: 'none', border: 'none', cursor: 'pointer' }}>
                Show all stores
              </button>
            )}
          </div>
                ...
                <div key={p.id || i} style={{ padding: '10px 16px', borderBottom: '1px solid #F9FAFB', display: 'flex', gap: 10, alignItems: 'flex-start', transition: 'background 120ms' }}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' as const }}>
                      ...
                      {isNewProduct(p) && (
                        <span style={{ fontSize: 9, fontWeight: 700, color: '#4f8ef7', background: 'rgba(79,142,247,0.08)', padding: '1px 6px', borderRadius: 10 }}>NEW TODAY</span>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div style={{ padding: '12px 14px' }}>
            ...
            {products.filter(p => p.category).slice(0, 8).map((p: any, i: number) => (
              <div key={i} style={{ padding: '8px 0', borderBottom: '1px solid #F9FAFB', display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                <span style={{ fontSize: 14, flexShrink: 0, color: '#4f8ef7' }}>●</span>
                <div>
              ...
              {sortedAds.map(ad => (
                ...
                >
                  <div style={{ height: 180, background: ad.bg || '#F9FAFB', position: 'relative', display: 'flex', alignItems: 'flex-start', justifyContent: 'flex-start', padding: 16, flexDirection: 'column', gap: 8 }}>
                    ...
                    <div style={{ marginTop: 'auto', display: 'inline-block', background: ad.accent || '#4f8ef7', color: 'white', fontSize: 11, fontWeight: 700, padding: '4px 12px', borderRadius: 99 }}>{ad.cta || 'Shop Now'}</div>
                  ...
                  <div style={{ padding: 16 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      <button onClick={() => setViewAd(ad)} style={{
                        ...
                        onMouseEnter={e => { e.currentTarget.style.background = 'rgba(79,142,247,0.08)'; e.currentTarget.style.transform = 'scale(1.02)'; }}
                      ...
                      <button
                        ...
                        style={{
                          ...
                          color: savedAds.has(ad.id) ? '#4f8ef7' : '#D1D5DB',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ height: 300, background: viewAd.bg || '#F9FAFB', position: 'relative', display: 'flex', flexDirection: 'column', gap: 12, padding: 24, justifyContent: 'center' }}>
              ...
              <div style={{ display: 'inline-block', background: viewAd.accent || '#4f8ef7', color: 'white', fontSize: 12, fontWeight: 700, padding: '6px 16px', borderRadius: 99, alignSelf: 'flex-start' }}>{viewAd.cta || 'Shop Now'}</div>
              <div style={{
