...
export default function SharedReport() {
  ...
  if (loading) return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', color: '#6B7280', fontSize: 14 }}>Loading report...</div>;
  if (error) return (
    <div style={{ display: 'flex', flexDirection: 'column' as const, justifyContent: 'center', alignItems: 'center', minHeight: '100vh', gap: 12 }}>
      ...
      <div style={{ fontFamily: brico, fontWeight: 700, fontSize: 18, color: '#0A0A0A' }}>Report not found</div>
  ...
  return (
    <div style={{ background: '#FAFAFA', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 24px' }}>
        <h1 style={{ fontFamily: brico, fontWeight: 800, fontSize: 24, color: '#0A0A0A', marginBottom: 4 }}>{report.title}</h1>
        ...
        {products.map((p: any, i: number) => (
          <div key={i} style={{ background: 'white', border: '1px solid #E5E7EB', borderRadius: 12, padding: 20, marginBottom: 12 }}>
            <div style={{ display: 'flex', gap: 14 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: brico, fontWeight: 700, fontSize: 15, color: '#0A0A0A', marginBottom: 6 }}>{p.product_title}</div>
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' as const }}>
                  ...
                  {p.est_monthly_revenue_aud && <div style={{ fontSize: 12, color: '#0A0A0A', fontWeight: 600 }}>~${p.est_monthly_revenue_aud}/mo</div>}
        ...
        <div style={{ background: '#EEF2FF', border: '1px solid #C7D2FE', borderRadius: 16, padding: '32px 24px', textAlign: 'center' as const, marginTop: 32 }}>
          <div style={{ fontFamily: brico, fontWeight: 800, fontSize: 20, color: '#0A0A0A', marginBottom: 8 }}>Want more product insights?</div>
          <div style={{ fontSize: 14, color: '#6B7280', marginBottom: 20 }}>Majorka finds winning products before your competitors. Free to try.</div>
