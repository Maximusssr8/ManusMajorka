...
const C = { bg: '#FAFAFA', surface: 'white', border: '#E5E7EB', gold: '#4f8ef7', text: '#0A0A0A', muted: '#6B7280' };
...
function ShopTypeBadge({ type }: { type: string }) {
  const colors: Record<string, { bg: string; text: string }> = {
    'dropship': { bg: 'rgba(79,142,247,0.12)', text: '#4f8ef7' },
    'brand': { bg: 'rgba(79,142,247,0.18)', text: '#4f8ef7' },
    'print-on-demand': { bg: 'rgba(79,142,247,0.12)', text: '#a5b4fc' },
  };
      ...
      <div style={{ padding: '0 0 28px' }}>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
          ...
          {shopTab === 'niches' && (() => {
            ...
            const oppColor = (o: number) => o >= 90 ? '#4f8ef7' : o >= 80 ? '#059669' : '#D97706';
            return (
          ...
          {shopTab === 'trends' && (
            <div style={{ padding: 24 }}>
              ...
              {(() => {
                const trendData = [
                  { name: 'Pet GPS Trackers', color: '#4f8ef7', data: [180, 210, 195, 240, 260, 310, 350] },
            ...
            <div style={{ padding: 24 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 16 }}>
                {[
                  ...
                  { season: 'Winter', months: 'Jun – Aug', emoji: '❄️', categories: ['Weighted Blankets', 'Indoor Fitness', 'Heating & Warmth', 'Hot Beverages'], color: '#4f8ef7' },
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
        ...
        {loading ? (
          ...
          <div key={shop.id}
            ...
            onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.borderLeft = `3px solid ${C.gold}`; (e.currentTarget as HTMLDivElement).style.background = 'rgba(79,142,247,0.03)'; }}
          ...
          >
            ...
            <div style={{ fontSize: 13, fontWeight: 700, color: shop.growth_rate_pct >= 0 ? '#4f8ef7' : '#ef4444' }}>
            ...
            <button
              ...
              style={{ padding: '6px 14px', borderRadius: 6, background: 'rgba(79,142,247,0.1)', border: `1px solid rgba(79,142,247,0.25)`, color: C.gold, fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: "'Syne', sans-serif", whiteSpace: 'nowrap' }}
            >
              Analyse →
            </button>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 20 }}>
          {Array.from({ length: Math.min(pages, 5) }, (_, i) => i + 1).map(p => (
            <button key={p} onClick={() => fetchShops(p)}
              style={{ width: 36, height: 36, borderRadius: 8, border: `1px solid ${page === p ? C.gold : C.border}`, background: page === p ? 'rgba(79,142,247,0.1)' : 'transparent', color: page === p ? C.gold : C.muted, fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>
              {p}
            </button>
          ))}
        </div>
