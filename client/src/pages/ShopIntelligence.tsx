...
const C = { bg: '#FAFAFA', surface: 'white', border: '#E5E7EB', gold: '#3B82F6', text: '#0A0A0A', muted: '#6B7280' };
...
function ShopTypeBadge({ type }: { type: string }) {
  const colors: Record<string, { bg: string; text: string }> = {
    'dropship': { bg: 'rgba(59,130,246,0.12)', text: '#3B82F6' },
    'brand': { bg: 'rgba(59,130,246,0.18)', text: '#3B82F6' },
    'print-on-demand': { bg: 'rgba(59,130,246,0.12)', text: '#93C5FD' },
  };
    ...
    <div style={{ minHeight: '100vh', background: C.bg, padding: isMobile ? '16px' : '32px 24px' }}>
      ...
      <div style={{ padding: '0 0 28px' }}>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
          ...
          {shopTab === 'niches' && (() => {
            ...
            const oppColor = (o: number) => o >= 90 ? '#3B82F6' : o >= 80 ? '#059669' : '#D97706';
            return (
          ...
          {shopTab === 'trends' && (
            <div style={{ padding: 24 }}>
              ...
              {(() => {
                const trendData = [
                  { name: 'Pet GPS Trackers', color: '#3B82F6', data: [180, 210, 195, 240, 260, 310, 350] },
            ...
            <div style={{ padding: 24 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 16 }}>
                {[
                  ...
                  { season: 'Winter', months: 'Jun – Aug', emoji: '❄️', categories: ['Weighted Blankets', 'Indoor Fitness', 'Heating & Warmth', 'Hot Beverages'], color: '#3B82F6' },
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 20, padding: '16px', background: C.surface, borderRadius: 12, border: `1px solid ${C.border}` }}>
        <select value={niche} onChange={e => setNiche(e.target.value)}
          style={{ padding: '8px 12px', borderRadius: 7, background: '#0d0d10', border: `1px solid ${C.border}`, color: C.text, fontSize: 13, cursor: 'pointer' }}>
        ...
        <input
          ...
          style={{ flex: 1, minWidth: 160, padding: '8px 12px', borderRadius: 7, background: '#0d0d10', border: `1px solid ${C.border}`, color: C.text, fontSize: 13, outline: 'none' }}
        />
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
        ...
        {loading ? (
          ...
          <div key={shop.id}
            ...
            onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.borderLeft = `3px solid ${C.gold}`; (e.currentTarget as HTMLDivElement).style.background = 'rgba(59,130,246,0.03)'; }}
          ...
          >
            ...
            <div style={{ fontSize: 13, fontWeight: 700, color: shop.growth_rate_pct >= 0 ? '#3B82F6' : '#ef4444' }}>
            ...
            <button
              ...
              style={{ padding: '6px 14px', borderRadius: 6, background: 'rgba(59,130,246,0.1)', border: `1px solid rgba(59,130,246,0.25)`, color: C.gold, fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: "'Bricolage Grotesque', sans-serif", whiteSpace: 'nowrap' }}
            >
              Analyse →
            </button>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 20 }}>
          {Array.from({ length: Math.min(pages, 5) }, (_, i) => i + 1).map(p => (
            <button key={p} onClick={() => fetchShops(p)}
              style={{ width: 36, height: 36, borderRadius: 8, border: `1px solid ${page === p ? C.gold : C.border}`, background: page === p ? 'rgba(59,130,246,0.1)' : 'transparent', color: page === p ? C.gold : C.muted, fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>
              {p}
            </button>
          ))}
        </div>
