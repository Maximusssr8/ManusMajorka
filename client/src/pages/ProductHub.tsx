...
export default function ProductHub() {
  ...
  const { data: product, isLoading: loadingProduct } = trpc.products.get.useQuery(
    { id: productId },
    { enabled: !!productId }
  );
  const { data: outputs, isLoading: loadingOutputs } = trpc.savedOutputs.list.useQuery(
    { productId },
    { enabled: !!productId }
  );
  ...
  const deleteMut = trpc.savedOutputs.delete.useMutation({
    onSuccess: () => {
      utils.savedOutputs.list.invalidate({ productId });
  ...
  const outputsByStage = stageOrder.reduce(
    (acc, stage) => {
    ...
    >
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          <button
            onClick={() => navigate('/app/my-products')}
            className="p-2 rounded-lg hover:bg-[#0d0d10]/5 transition-all"
          ...
          <div>
            <div className="flex items-center gap-2">
              <Package size={16} style={{ color: stageColors[product.status] || '#4f8ef7' }} />
            ...
            return (
              ...
              >
                <button
                  ...
                  className="w-full flex items-center justify-between p-4 transition-all hover:bg-[#0d0d10]/3"
                    ...
                    {stageOutputs.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {stageOutputs.map((output) => {
                          ...
                          let parsed: any = null;
                          try {
                            parsed = JSON.parse(output.outputJson);
                          } catch {}
                          return (
                            <div
                              key={output.id}
                            ...
                            >
                              <div
                                ...
                                onClick={() => setExpandedOutput(isOpen ? null : output.id)}
                              >
                                <div className="flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-1">
                                  <button
                                    ...
                                    className="p-1 rounded hover:bg-[#0d0d10]/5 transition-all"
