      ...
      >
        ...
        <div className="px-5 pt-6 pb-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold uppercase tracking-widest" style={{ color: "rgba(139,92,246,0.7)", fontFamily: "Syne, sans-serif" }}>
              Competitive Intel
            </span>
          </div>
        ...
        <div className="px-5 pb-6 space-y-2">
          <button
            ...
            style={{
              ...
              background: isGenerating ? "rgba(139,92,246,0.15)" : "linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%)",
        ...
        {!result && !isGenerating && (
          <div className="flex flex-col items-center justify-center h-full px-8 text-center">
            ...
            <p className="text-sm max-w-md" style={{ color: "rgba(240,237,232,0.4)" }}>
              Enter your niche and let AI map the competitive landscape — market size, competitor analysis, opportunity gaps, pricing intelligence, and entry strategy.
            </p>
        ...
        {isGenerating && (
          <div className="flex flex-col items-center justify-center h-full">
            <div className="relative w-16 h-16 mb-6">
              ...
              <div className="relative w-16 h-16 rounded-full flex items-center justify-center" style={{ background: "rgba(139,92,246,0.1)", border: "1px solid rgba(139,92,246,0.3)" }}>
                <Sparkles size={24} style={{ color: "#8b5cf6" }} className="animate-pulse" />
              </div>
            </div>
            <p className="text-sm font-semibold" style={{ color: "#8b5cf6" }}>Scanning the market...</p>
            <p className="text-xs mt-1" style={{ color: "rgba(240,237,232,0.3)" }}>Analysing competitors, gaps, and opportunities</p>
          </div>
        )}
          ...
          <div className="max-w-3xl mx-auto px-6 py-8">
            ...
            <div className="mb-8 pb-6" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
              ...
              <div className="flex items-center gap-3 mb-3">
                <h2 className="text-2xl font-black" style={{ fontFamily: "Syne, sans-serif", color: "#f0ede8" }}>
                  {niche}
                </h2>
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full" style={{
                  background: result.marketOverview.trend === "growing" ? "rgba(45,202,114,0.1)" : result.marketOverview.trend === "declining" ? "rgba(239,68,68,0.1)" : "rgba(245,158,11,0.1)",
                  ...
                  <span className="text-xs font-semibold capitalize" style={{ color: result.marketOverview.trend === "growing" ? "#2dca72" : result.marketOverview.trend === "declining" ? "#ef4444" : "#f59e0b" }}>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={label} className="rounded-xl px-3 py-2.5" style={{ background: "rgba(139,92,246,0.06)", border: "1px solid rgba(139,92,246,0.15)" }}>
                    <div className="text-xs font-semibold uppercase tracking-wider mb-1" style={{ color: "rgba(139,92,246,0.6)" }}>{label}</div>
                    <div className="text-sm font-bold" style={{ color: "#f0ede8" }}>{value}</div>
                  </div>
                ))}
              </div>
            </div>
                ...
                <ul className="space-y-1.5">
                  {result.marketOverview.keyDrivers.map(driver => (
                    <li key={driver} className="flex items-start gap-2">
                      ...
                      <span className="text-sm" style={{ color: "#f0ede8" }}>{driver}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {/* Competitors */}
            <SectionCard title={`Competitor Analysis (${result.competitors.length})`} accent="#ef4444">
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-black" style={{ fontFamily: "Syne, sans-serif", color: "#f0ede8" }}>{comp.name}</span>
                          <CopyBtn text={comp.name} />
                        </div>
                        <div className="flex items-center gap-3 mt-1">
                          <span className="text-xs" style={{ color: "rgba(240,237,232,0.4)" }}>Rev: {comp.estimatedRevenue}</span>
                          <span className="text-xs" style={{ color: "rgba(240,237,232,0.4)" }}>Price: {comp.priceRange}</span>
                        </div>
                      </div>
                    </div>
                    <p className="text-xs mb-3" style={{ color: "rgba(240,237,232,0.5)", fontStyle: "italic" }}>"{comp.uniqueAngle}"</p>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <span className="text-xs font-semibold uppercase tracking-wider block mb-1.5" style={{ color: "rgba(45,202,114,0.6)" }}>Strengths</span>
                        <ul className="space-y-1">
                          {comp.strengths.map(s => (
                            <li key={s} className="text-xs flex items-start gap-1.5" style={{ color: "rgba(240,237,232,0.6)" }}>
                      ...
                      <div>
                        <span className="text-xs font-semibold uppercase tracking-wider block mb-1.5" style={{ color: "rgba(239,68,68,0.6)" }}>Weaknesses</span>
                        <ul className="space-y-1">
                          {comp.weaknesses.map(w => (
                            <li key={w} className="text-xs flex items-start gap-1.5" style={{ color: "rgba(240,237,232,0.6)" }}>
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {comp.marketingChannels.map(ch => (
                        <span key={ch} className="text-xs px-2 py-0.5 rounded-full" style={{ background: "rgba(255,255,255,0.06)", color: "rgba(240,237,232,0.5)", border: "1px solid rgba(255,255,255,0.1)" }}>
                          {ch}
                        </span>
                      ))}
                    </div>
              ...
              <div className="space-y-3">
                {result.opportunityGaps.map((opp, i) => (
                  <div
                    key={i}
                    className="rounded-xl p-4"
                    style={{ background: "rgba(45,202,114,0.04)", border: "1px solid rgba(45,202,114,0.12)" }}
                  >
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <span className="text-sm font-semibold flex-1" style={{ color: "#f0ede8" }}>{opp.gap}</span>
                      <DifficultyBadge difficulty={opp.difficulty} />
                    </div>
                    <div className="flex items-center gap-2">
                      <TrendingUp size={12} style={{ color: "#2dca72" }} />
                      <span className="text-xs" style={{ color: "rgba(45,202,114,0.8)" }}>{opp.potentialImpact}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <SectionCard title="Pricing Intelligence" accent="#f59e0b">
              <div className="grid grid-cols-4 gap-2 mb-4">
                {[
                  ...
                  { label: "Market High", value: result.pricingIntelligence.marketHigh, color: "#ef4444" },
                  { label: "Sweet Spot", value: result.pricingIntelligence.sweetSpot, color: "#d4af37" },
                ].map(({ label, value, color }) => (
                  <div key={label} className="rounded-xl px-3 py-2.5 text-center" style={{ background: `${color}08`, border: `1px solid ${color}20` }}>
                    <div className="text-xs font-semibold uppercase tracking-wider mb-1" style={{ color: `${color}80` }}>{label}</div>
                    <div className="text-sm font-black" style={{ color }}>{value}</div>
                  </div>
                ))}
              </div>
              <div className="flex items-start gap-3">
                <p className="text-sm leading-relaxed flex-1" style={{ color: "#f0ede8" }}>{result.pricingIntelligence.pricingStrategy}</p>
            ...
            <SectionCard title="Customer Sentiment" accent="#06b6d4">
              <div className="space-y-4">
                {[
                  { label: "Top Complaints", items: result.customerSentiment.topComplaints, color: "#ef4444", icon: "😤" },
                  { label: "What They Love", items: result.customerSentiment.topPraises, color: "#2dca72", icon: "😍" },
                  { label: "Unmet Needs", items: result.customerSentiment.unmetNeeds, color: "#06b6d4", icon: "💡" },
                ].map(({ label, items, color, icon }) => (
                  <div key={label}>
                    <div className="flex items-center gap-2 mb-2">
                      <span>{icon}</span>
                      <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: `${color}80` }}>{label}</span>
                    </div>
                    <ul className="space-y-1.5">
                      {items.map(item => (
            ...
            <SectionCard title="Strategy & Entry Plan" accent="#d4af37">
              <div className="mb-4">
                ...
                <div className="space-y-2">
                  {result.strategicRecommendations.map((rec, i) => (
                    <div key={i} className="flex items-start gap-3 px-4 py-3 rounded-xl" style={{ background: "rgba(212,175,55,0.05)", border: "1px solid rgba(212,175,55,0.12)" }}>
                      <span className="text-xs font-black flex-shrink-0 mt-0.5" style={{ color: "rgba(212,175,55,0.5)" }}>{String(i + 1).padStart(2, "0")}</span>
                      <span className="text-sm flex-1" style={{ color: "#f0ede8" }}>{rec}</span>
                      <CopyBtn text={rec} />
                    </div>
                  ))}
                </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex items-start gap-3">
                  <p className="text-sm leading-relaxed flex-1" style={{ color: "#f0ede8" }}>{result.threatAssessment}</p>
              ...
              <div>
                ...
                <div className="flex items-start gap-3 px-4 py-3 rounded-xl" style={{ background: "rgba(212,175,55,0.08)", border: "1px solid rgba(212,175,55,0.2)" }}>
                  <p className="text-sm leading-relaxed flex-1" style={{ color: "#f0ede8" }}>{result.entryStrategy}</p>
